...
### PIPELINE-DRIFT-SITES-PATRIMOINE-001 — Drift pipeline vs fichier publié

**Constat (11 mai 2026, audit Code sprint curation N1) :**
- `consolidate_sites.py` écrit dans un chemin sandbox hardcodé non applicable en local (`/sessions/busy-awesome-mayer/...`).
- `docs/data/sites_patrimoine.json` (453 sites avant sprint) est édité par autres scripts (`sync_cross_app.py`, `brief_pipeline.py`) ou manuellement (briefs 35/38/39/40).
- Diff 479 (canonical) → 453 (publié) = 26 sites perdus, dont 4 illustrés réintégrés manuellement dans ce sprint (commit `18de84e`). **22 autres sites perdus, identité inconnue.**

**Mise à jour (12 mai 2026, sprint micro-data) :**
- Audit visuels patrimoine 2026-05-12 a identifié 9 nouveaux visuels orphelins (slugs présents au canonical mais absents de `sites_patrimoine.json`) : `capu_rossu`, `cirque_de_bonifato`, `foret_de_tartagine`, `foret_de_valdu_niellu`, `grotte_de_bonifacio`, `lac_de_tolla`, `massif_de_l_ospedale`, `piscia_di_gallo`, `plateau_du_coscione`. Réintégrés manuellement (commit `6094c11`), corpus Phase 1 passe 171 → 180.
- **Total sites perdus cumulés : 4 (curation N1) + 9 (micro-data) + 1 (san_andrea_de_piedicroce, brief séparé futur) = 14 sites en 2 mois.** Patches manuels itératifs.
- Le pipeline n'est toujours pas la source de vérité runtime.

**Risque :** toute modification data via le pipeline canonique ne se propage pas au runtime. Les corrections sont éphémères tant que la chaîne n'est pas réparée. Risque de drift continu.

**Action requise (sprint dédié à planifier) :**
- Réparer `consolidate_sites.py` pour écrire au bon chemin (cible publique `docs/data/sites_patrimoine.json`)
- Auditer les ~13 sites encore perdus (identité inconnue post-réintégrations cumulées : 26 perdus initialement − 13 réintégrés = ~13 résiduels)
- Décider d'une seule source de vérité (canonical `sites_corse.json` vs publié `sites_patrimoine.json`)
- Workflow CI : test cohérence canonical vs publié avant merge

**Priorité : HAUTE** (escaladée depuis « moyenne » au sprint micro-data 2026-05-12). Justification : 14 sites perdus cumulés en 2 mois, patches manuels itératifs, pipeline n'est toujours pas la source de vérité. À planifier avant qu'il y ait encore plus de drift.
...
### PIPELINE-DRIFT-SITES-EM-CORSE-001 — Drift `sites_em.json` vs `sites_corse.json`

**Constat (11 mai 2026, audit Code sprint app-ui-polish) :**
Le layer `sitesRemarq` d'`app.html` fetch `docs/data/sites_em.json` (48 sites). Mais `docs/data/sites_corse.json` (canonical 479 sites post-Brief 33 split, target deprecation 2026-06-05) contient les mêmes entrées avec des coordonnées parfois différentes :

| Slug | sites_em.json | sites_corse.json | Drift |
|---|---|---|---|
| `barrage_padula` | (42.62806, 9.32278) | identique | 0m, MAIS coord erronée dans les 2 (devrait être Fium'Orbu sud-est) |
| `calanques_de_piana` | (42.2529, 8.6575) | (42.2525, 8.65722) | ~30m (négligeable) |
| `capu_rossu` | (42.24028, 8.58667) → corrigé (42.225, 8.573) | idem corrigé | resync OK |
| `min_argentella` | (42.46076, 8.70999) | (42.354, 8.882) | **>10km drift**, les 2 fausses |
| `reserve_de_scandola` | (42.3589, 8.5615) → corrigé (42.36072, 8.56127) | (42.36072, 8.56127) | aligné OK |

**Cause probable :** `sites_em.json` édité out-of-band sans re-propagation depuis le canonical. Même cause racine que `PIPELINE-DRIFT-SITES-PATRIMOINE-001`.

**Action requise (sprint séparé) :**
- Réparer pipeline `consolidate_sites.py` pour générer `sites_em.json` depuis canonical avec règles de filtrage Phase 1 EM
- Auditer tous les sites pour drift coord/champ (au-delà des 5 identifiés)
- Une seule source de vérité, un seul fichier de sortie par layer
- **Re-sourcer** `barrage_padula` (Wikipedia FR / OSM / EDF SEI) et `min_argentella` (OSM Argentella / BRGM mines fermées / Persée article minier Crovani-Argentella) — les 2 sites ont été SKIP dans le sprint app-ui-polish faute de source externe accessible côté agent

**Priorité :** moyenne. Pas bloquant Phase 1 mais à traiter avec `PIPELINE-DRIFT-SITES-PATRIMOINE-001` (même cause).
...
### ALERIA-RUINE-FICHE-V3-KEY-MISMATCH-001 — `fiche_v3_slug` invalide dans `aleria_antique`

**Description :** Dans `docs/data/sites_patrimoine.json`, le site `aleria_antique` a `fiche_v3_slug: "aleria_ruine"` (1 seul site sur 457 avec ce champ rempli). Or la clé réelle dans `docs/data/fiches_patrimoine.json` est `aleria_antique`, pas `aleria_ruine`. Le lookup `fiches[site.fiche_v3_slug]` retourne donc `null`, et le code retombe sur le fallback `fiches[site.slug]` (`aleria_antique`) qui lui fonctionne. Bug latent masqué par le fallback : la carte postale s'affiche correctement par accident.

**Priorité :** Basse (fonctionne par fallback).

**Condition de déblocage :** Choisir une des 2 corrections :
- (a) Modifier `fiche_v3_slug: "aleria_ruine"` → `"aleria_antique"` dans `sites_patrimoine.json` (1 ligne)
- (b) Renommer la clé `aleria_antique` → `aleria_ruine` dans `fiches_patrimoine.json`

L'option (a) est cohérente avec la convention (`fiche_v3_slug = clé fiche`), l'option (b) modifie 1 clé JSON mais nécessite cohérence inter-fichiers. Identifiée sprint curation N1 (2026-05-11), Item 7.
