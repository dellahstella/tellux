...
**Dernière mise à jour :** 21 mai 2026 (vague 2 corrections PIP — branche `data/pip-corrections-vague-2`) — Application consolidée : 21 corrections `pieve_slug` B1, retrait de 2 sites inconnus du corpus (`san_giovanni_de_coggia`, `san_giovanni_de_santa_maria_siche`), merge Stazzona di u Diavulu → Dolmen Fontanaccia (suppression de `stazzona_di_u_diavulu_menhirs`), merge Farinole (`ophi_farinole` supprimé, `mine_de_magnetite_de_farinole` enrichi + GPS corrigé + reclassé « Site naturel remarquable »), correction GPS Pont d'Altiani (source AFGC), ajout du site `mine_amiante_olmeta_du_cap` (gisement amiante BRGM 80-RDM-003-FE). Phase 1 : 190 → 187 sites. Fermeture de `CORPUS-DOUBLONS-FARINOLE-001` ; ouverture de 4 dettes (`CORPUS-OPHI-PREFIXE-001`, `OLMETA-CAP-GPS-AFFINAGE-001`, `OLMETA-CAP-PIEVE-MAPPING-001`, `OLMETA-CAP-VISUEL-001`). Précédente : 21 mai 2026 (clôture session pré-FEDER 2026-05-18 — mise à jour docs de suivi) — Ouverture de la dette de suivi `PATRIMOINE-PIEVES-V7-RESHAPE-INSPECTION-001` (basse priorité — 11 pieves reshapées par le rebuild v7, inspection visuelle Soleil à faire, aucun bug identifié). Création de `docs/operations/ETAT_FIN_SESSION_2026-05-18.md` et `docs/operations/HANDOFF_NEXT_SESSION_2026-05-18.md`. Aucune autre dette modifiée — les dettes fermées par le sprint pré-FEDER (Stratégie D, Étapes 1/3/5) ont été clôturées dans leurs PRs respectives. Précédente : 21 mai 2026 (Étape 5 PR C — D3 rebuild voie-a + D4 containment renforcé) — Régénération de `docs/data/pieves_polygons.json` par la voie-a : câblage du mapping v4 dans `build_pieves_polygons.py`, rebuild → `version v7-cleanup-mapping-amont-rebuild-2026-05-18`, 50 pieves, `stats.pieves_count` 51→50 et `total_communes` 347→360 auto-corrigés, surface 8703.3 km² (écart <0,01 % vs prod), override `pieve_bozio` préservé, sortie minifiée. **11 pieves changent de composition** vs la prod (carte v4 cohérente vs prod incohérente post-voie-b — refonte assumée, arbitrage Soleil 2026-05-21). Ajout du containment check renforcé mapping↔dérivé (`--strict-mapping`, mode warn par défaut) : rebuild actuel 0 écart. **Clôture de 4 dettes** : `PIEVE-MAPPING-AMONT-DESYNCHRO-001` (HAUTE — voie-a redevenue viable), `POLYGONE-INVALID-SELF-INTERSECTING-001` (rebuild → 0 polygone invalide), `COMMUNES-COUNT-OBSOLETE-POST-VOIE-B-001` (compteurs recalculés cohérents), `MAPPING-PRE-STRATD-AD-HOC-TRANSFERS-UNDOCUMENTED-001` (rebuild v4 best-effort accepté comme nouvelle référence). `MAPPING-PREQW-ORIGIN-UNKNOWN-001` reste ouverte. Chantier Étape 5 (D1+D2+D3+D4) terminé. Précédente : 20 mai 2026 (Étape 5 PR B — D2 archivage scripts ad hoc) — Archivage de 5 scripts one-shot historiques (`phase_d3_pieves`, `phase_qw_pieves`, `phase_r3_rename_ajaccio`, `phase_strat_d_patch_derive`, `phase_strat_d_phase2_splits`) vers `scripts/archive/phase_oneshots/` (en-tête d'archivage + README) — aucun n'était référencé par la CI ni importé. Création rétroactive de `phase_strat_d_etape3_fusion_lota.py` (script idempotent documentant la fusion `pieve_bastia` + `pieve_brando` → `pieve_lota` exécutée par PR #648). Intégration dans `build_pieves_polygons.py` des métadonnées de l'ex-`phase_qw_pieves.py` : `QW_DIOCESES_FALLBACK` (diocese_medieval de secours, no-op si déjà renseigné) + strip générique du préfixe « Pieve di / d' ». Build vérifié exécutable end-to-end (exit 0). `phase_strat_d_retag_sites.py` et `phase_strat_d_phase2_retag_sites.py` conservés (retag idempotents). Aucune nouvelle dette ; aucune modification du dérivé prod. `PIEVE-MAPPING-AMONT-DESYNCHRO-001` reste ouverte (clôture en PR C). Précédente : 20 mai 2026 (Étape 5 PR A — D1 cleanup mapping amont, brief REV2) — Alignement du mapping amont sur la prod 50 pieves. Création de `_drafts/pieves_communes_mapping_v4_cleanup_2026-05-18.json` (incrémental v4 sur modèle v2/v3) : suppression de 7 zombies (`ampugnani`, `caccia`, `campoloro`, `giovellina`, `balagne`, `bastia`, `brando`), déclaration explicite de 8 pieves manquantes (`piana`, `sagone`, `aregno`, `calenzana`, `ostriconi`, `castagniccia`, `lota`, `zicavo`), 29 transferts. Source de vérité : scripts ad hoc `phase_strat_d_*.py` (listes explicites pour 6/8 pieves) ; reconstitution triviale (`lota` = ex-bastia + ex-brando) et PIP polygones prod best-effort (`zicavo` + 26 communes ex-zombies pré-Strat-D) pour le reste — arbitrage Soleil 2026-05-20. Méthode REV2 : la prémisse REV1 « PIP = source de vérité unique » a été abandonnée après blocage Phase A (16/50 `communes_count` prod obsolètes, 3 polygones invalides). `scripts/validate_mapping.py` REV2 créé (cohérence interne du mapping : 0 zombie, 0 manquante, 360 communes mappées). Suppression de la référence zombie `pieve_caccia` de `PIEVE_DOYENNES_OVERRIDES.json` (9 → 8 entrées). **Aucune modification du dérivé prod** `pieves_polygons.json` (rebuild en PR C). Ajout 4 dettes : `MAPPING-PREQW-ORIGIN-UNKNOWN-001`, `POLYGONE-INVALID-SELF-INTERSECTING-001`, `COMMUNES-COUNT-OBSOLETE-POST-VOIE-B-001`, `MAPPING-PRE-STRATD-AD-HOC-TRANSFERS-UNDOCUMENTED-001`. `PIEVE-MAPPING-AMONT-DESYNCHRO-001` reste ouverte (clôture en PR C après rebuild validé). Précédente : 20 mai 2026 (B-ZONES Étape 4 sprint pré-FEDER — PR B Tier 2) — 8 sites Tier 2 favorables transformés en zones polygonales : 5 monts (`monte_cinto`, `monte_san_petrone`, `monte_stello`, `monte_d_oro`, `monte_renoso`, octogones manuels rayon ~2-3 km autour du sommet), 2 OSM (`capu_rossu` relation 9376817 natural=cape, `lac_de_nino` way 28890021 + buffer ~600 m pozzines), 1 pointe manuelle (`cap_corse_extreme_nord` / Capu Bianchi). Compteur B-ZONES : 15 → **23** zones actives. Smoke test PIP Corse 23/23 verts. Fermeture de la dette `PATRIMOINE-B-ZONES-TIER2-FAVORABLE-001`. Aucune dette `pieve` ou `doyenne` modifiée. Précédente : 20 mai 2026 (B-ZONES Étape 4 sprint pré-FEDER — PR A Tier 1) — Transformation de 15 sites naturels en zones polygonales interactives au survol (hover desktop fade 200 ms, tap mobile 1500 ms). Ajout du schéma `is_zone` / `zone_geometry` (GeoJSON `[lon, lat]`) / `zone_source` / `zone_simplification_pts` sur 15 sites de `sites_patrimoine.json` ; renderer Leaflet `.tlx-zone-hover` ocre `#C28533` (fillOpacity 0.3) dans `patrimoine.html` ; script `scripts/b_zones_smoke_test.py` (PIP bbox Corse + containment doyenné, 15/15 verts). 6 polygones OSM (Scandola RNN, Vizzavona, Tartagine, Valdu Niellu, Bonifatu, RBI Tavignano) + 9 tracés manuels (Agriates, Calanche de Piana, Aiguilles de Bavella, Spelunca, Inzecca, Lancône, Haut-Asco restreint, Ospedale, Coscione). `massif_du_haut_asco` : tracé manuel restreint à la haute vallée (arbitrage Soleil — pas la commune entière d'Asco). Ajout 3 dettes : `PATRIMOINE-B-ZONES-TIER2-FAVORABLE-001` (8 sites PR B), `PATRIMOINE-B-ZONES-T2-DEFAVORABLE-001` (3 sites pin assumés), `PATRIMOINE-B-ZONES-ETANGS-CORPUS-001` (étangs Diane/Urbino/Biguglia absents du corpus). Aucune dette `pieve` ou `doyenne` modifiée (zones = sites). Précédente : 18 mai 2026 (Étape 3 sprint pré-FEDER — pieve_lota + labels diocese) — Sujets 1+2+3 traités en 1 PR groupée. Sujets 1+2 : suffixe ` (médiéval)` au label diocese affiché en popup pieve legacy (clarification ambiguïté slug pieve ↔ diocese sans rename slug interne) + fix `pieve_patrimonio` (diocese_medieval `"?"` → `"Nebbiu"`, name `"pieve_patrimonio"` → `"Patrimonio"`). Sujet 3 : refonte `pieve_bastia` + `pieve_brando` en `pieve_lota` élargie (7 communes / 16 sites, Option A audit Cowork validée Soleil). Polygon UNION shapely (~124 km²), diocese Mariana, doyenne_du_cap. Hameaux préservés dans `note_rattachement` (Toga/Lupino/Cardo, Miomo, Erbalunga). 2 aliases v4 ajoutés (`pieve_bastia → pieve_lota`, `pieve_brando → pieve_lota`). 16 sites retag (14 simple + 2 anomalies double-retag pieve+doyenne via `BP-FIX-RATTACHEMENT-COMPLET-001` : `oratoire_santa_croce_bastia_haute_bastia_citadelle` ex-pieve_nebbiu/doy_golo et `san_giovanni_bastia_terra_vecchia` ex-pieve_biguglia/doy_golo, effet cascade Phase 1). Compteurs : 51→50 pieves, 3→5 aliases. Aucune nouvelle dette ouverte. Précédente : 18 mai 2026 (Sprint hygiène pré-FEDER, Étape 1) — 7 dettes traitées en 1 PR groupée : **3 résolues par correction data** (`CASTEDDU-BASTELICA-COORD-DOUTE-001` alignement metadata sur géo Altiani option a Soleil, `TOUR-ISOLELLA-INSEE-DISCORDANCE-001` INSEE 2A258→2A228 Pietrosella, `PIEVE-PIEDIPARTINO-INSEE-DISCORDANCE-001` INSEE 2B231→2B221 + retag pieve_orezza/doyenne_du_golo BP-FIX-RATTACHEMENT-COMPLET-001) + **3 fermées par invalidation/cosmétique** (`FARINOLE-COORD-DOUTE-001` coord cohérent Farinole, `PIEVE-VENACO-OVERFLOW-VISUEL-GEOMETRIQUE-001` cosmétique acceptable Phase 1 beta, `PIEVE-SORROINSU-CINARCA-POST-MIGRATION-PHASE2-001` équilibre 4+9 communes OK) + **sticker Balagne** documenté non-bug (image fonctionnelle, contraste cosmétique). Aucune nouvelle dette ouverte. Audit MCP residual mismatches visuel↔data : 1 → 0. Précédente : 18 mai 2026 (§8 mécanisme `doyenne_contemporain_override` + bozio) — Introduction d'un champ optionnel `doyenne_contemporain_override` dans `pieves_polygons.json` qui prime sur `doyenne_contemporain_majoritaire` dans toutes les lectures `patrimoine.html` (drill-down N2→N3 + structures dérivées). Résoud les cas doctrinaux où l'identité culturelle de la pieve diverge du ratio géographique calculé. Application à `pieve_bozio` (Cortenais doctrinal malgré PO 53% post-ingestion alesani Phase 1, foyer Paghjella). Compteurs : Cortenais 8→9, PO 7→6. Résolution 9 mismatches visuel↔data → 1 résiduel (`casteddu_bastelica` dette `CASTEDDU-BASTELICA-COORD-DOUTE-001` pré-existante hors-scope). Build script `scripts/build_pieves_polygons.py` préserve override + rationale sur rebuild (même pattern que `note_rattachement`, voie-b doctrine). Aucune dette nouvelle ouverte (mécanisme générique réutilisable pour cas similaires futurs). Précédente : 18 mai 2026 (cleanup fiches pieves, audit Cowork 2026-05-18) — 68 fiches markdown (34 pieves × v2 factuel + v3 sensoriel, vague 2026-05-03 et 2026-05-06) déplacées du repo public `tellux` vers le corpus privé `_corpus/fiches_pieves/` (Option A flat + sous-dossier `_archive_matiere_premiere/` pour `cap_corse` micro-régional). Doctrine ADR-001 acté + U1 Phase 1 (`note_rattachement` dans `pieves_polygons.json`, `fetchFiche` skippé pour pieves). Aucune fiche supprimée (toutes substantielles, valeur rédactionnelle pour Vague 4 post-FEDER). Volume retiré ~607 KB. Ajout 1 dette `PATRIMOINE-FICHES-PIEVES-VAGUE4-POST-FEDER-001` (rédiger fiches pour 19 pieves prod sans fiche : 3 créations Phase 1 + 6 Cap + 5 Casta restaurées + 5 existantes sans fiche). `_redirects` Cloudflare non ajouté (curl §3.2 non lancé, SPA fallback acceptable). Précédente : 18 mai 2026 (Stratégie D Phase 2 — splits vico (×3) + balagne (×3) : 47 → 51 pieves, +4 nouvelles (`pieve_piana` UNESCO Scandola, `pieve_sagone` golfe Sagone+Cargèse, `pieve_ostriconi` bassin versant NE Balagne, `pieve_calenzana` Calvi+arrière-pays) + rename `pieve_balagne` → `pieve_aregno` (alias v3). 99 sites retag (83 INSEE direct + 10 PIP fallback + 6 no-op). Cas particuliers : `san_pietro_letia` retag commune INSEE Q-2 ; `tour_d_isolella_sette_navi` conserve `pieve_ornano` Q-3 (dette `TOUR-ISOLELLA-INSEE-DISCORDANCE-001` ouverte). Ajout 3 dettes : `TOUR-ISOLELLA-INSEE-DISCORDANCE-001`, `PIEVE-SORROINSU-CINARCA-POST-MIGRATION-PHASE2-001`, `PIEVE-PIEDIPARTINO-INSEE-DISCORDANCE-001`. Voie-b patch direct du dérivé maintenue (cohérence Phase 1). Précédente : 18 mai 2026 (Stratégie D Phase 1 — containment fix pieves : split `pieve_mariana` en 3 (`pieve_biguglia` 11 communes, `pieve_balagne` +2, `pieve_castagniccia` 7 communes rename) + split `pieve_rogna` (création `pieve_altiani` 3 communes) + 29 transferts communes + 42 sites retag (28 brief + 13 ghost rescue + 1 revert valle_d_alesani). 47 pieves total (45→47). Voie (b) patch direct dérivé adoptée (voie a régénération produit 4 pieves zombies du mapping v1 désynchronisé). Fermeture `PATRIMOINE-PIEVE-MARIANA-MEGA-FUSION-001`. Ajout 5 dettes : `PATRIMOINE-PIEVE-BIGUGLIA-NEW-001`, `PATRIMOINE-PIEVE-ALTIANI-NEW-001`, `PATRIMOINE-PIEVE-CASTAGNICCIA-RENAME-001`, `PIEVE-VENACO-OVERFLOW-VISUEL-GEOMETRIQUE-001`, `PATRIMOINE-PIEVE-VEZZANI-PIETROSO-2B347-2B229-001`. **Escalade priorité `PIEVE-MAPPING-AMONT-DESYNCHRO-001` Moyenne → HAUTE** : PR cleanup mapping amont OBLIGATOIRE post-FEDER (4 pieves zombies + pieve_zicavo + pieve_patrimonio). Alias `pieve_mariana → pieve_castagniccia` ajouté. Containment check (`--strict-containment`) ajouté à `build_pieves_polygons.py`. Précédente : 17 mai 2026 (audit Cowork Phase B cat 1 + cat 2 : 17 orphelins retag selon géo (PATRIMOINE-ORPHANS-INVISIBLES-001 réduite 18 → 1 résiduel `cap_corse_extreme_nord` orphan transcommunal légitime) + 18 sites cross-doyennés retag selon géo (4 mariana in_pv=False vers pieves géo réelles, 10 non-mariana cohérents, 4 cas A option Soleil) + 2 cas suspendus option D (`tour_de_farinole` coord en mer probable, `casteddu_bastelica` doute homonymie). Doctrine actée Soleil : géo prime sur orthodoxie historique pour Phase 1 beta, Tellux n'est pas un atlas Casta strict, diocèse médiéval reste métadonnée informative mais ne pilote plus drill-down. Ajout 3 dettes : `FARINOLE-COORD-DOUTE-001`, `CASTEDDU-BASTELICA-COORD-DOUTE-001`, `PATRIMOINE-PIEVE-MARIANA-MEGA-FUSION-001` (refactor doctrinal passe par Cowork + ADR). Précédente : 15 mai 2026 (renommage `edifices_romans` → `patrimoine_religieux`, fermeture `EDIFICES-ROMANS-RENAME-001`). Précédente : 15 mai 2026 (clôture série fix N3 patrimoine post-implémentation) — série 14-15 mai 2026 : 1 implémentation (#577 niveau 3 drill-down pieve) + 15 paires fix→deploy (#579-608). 12 lots audit visuel Soleil traités par doyenné : Plaine Orientale, Cap, Golo, Balagne, Cortenais, Vico, Ajaccio, PTV, Extrême-Sud, Plaine Orientale fusions, Cap compléments. ~140 retags `pieve_slug` cumulés + 3 polygones supprimés (`pieve_ampugnani` PR #591, `pieve_campoloro` + `pieve_verde` PR #605) + 3 doublons supprimés (`patrimonio_paese`, `pont_d_altiani`, `tour_de_la_pietra_ile_rousse`). Ajout 2 BPs : `BP-FIX-RATTACHEMENT-COMPLET-001` (cohérence pieve_slug + doyenne_contemporain_slug) et `BP-FUSION-POLYGONE-ARBITRAGE-001` (fusion polygone admise sous arbitrage Soleil Phase A). Ajout 3 dettes nouvelles : `PATRIMOINE-PIEVE-NEBBIU-CHEVAUCHE-GOLO-CAP-001` (polygone large couvre 2 doyennés), `PATRIMOINE-DETROIT-BONIFACIO-ENTREE-MANQUANTE-001` (entité absente du data, ajout différé), `PATRIMOINE-PIEVE-CAP-NON-CLIQUABLE-UI-001` (bug visuel non reproductible programmatiquement). Fermeture `PATRIMOINE-HASH-DEEPLINK-CADRAGE-001` (résolu PR #577 commit `4dbee05`). Précédente : 14 mai 2026 (clôture Sprint 7 densification + correction rebalance doyenne_contemporain_slug) — ajout 2 dettes : `SITES-PATRIMOINE-PIEVES-SOUS-REPRESENTEES-COMPLEMENT-001` (7 pieves restantes après Sprint 7 : vivario, vallerustie, filosorma, ampugnani, ghisoni, tavagna, giovellina) et `SITES-PATRIMOINE-COMMUNES-MULTI-DOYENNES-EGALITE-001` (15 communes en égalité parfaite skippées par sprint correction rebalance PR #569 : Monte 4-doyennés, Altiani, Stazzona, Piedicorte-di-Gaggio, Castifao, Serra-di-Scopamène, Piazzole, Tavera, Coti-Chiavari 2-2, Pianello, San-Giuliano, Bocognano, Cervione 2-2, Penta-di-Casinca, Viggianello). Pré-audit cartographie 14 mai a révélé 8 pieves sous-rep (< 5 sites) → option B Soleil = rebalance d'abord puis Sprint 7 ajout. Rebalance 23 sites migrés (PR #569, doyenne_du_golo allégé -6) + 1 fix `san_lorenzo_de_ponte_leccia` commune Lecci → Morosaglia (Ponte-Leccia hameau). Sprint 7 ajout = +1 Pont de Muricciolu (Albertacce/pieve_niolu, GPS publié 42.326036/8.983954, XVIᵉ-XVIIIᵉ génois, emporté Ciaran 2023, inventaire Médiathèque Corse). pieve_niolu 3 → 5 sites (sortie zone sous-rep). 7 pieves restantes en dette pour sprint Cowork dédié futur. Précédente : 14 mai 2026 (clôture Sprint 6 paesi d'altura) — ajout 1 dette : `SITES-PATRIMOINE-PAESI-D-ALTURA-COMPLEMENT-001` (sous-cible 9/25 assumée scénario B, même doctrine que Sprint 4 ponts). Sprint 6 monolithique a créé l'axe `paesi_d_altura` (couleur DA `#A89B8C` Pierre claire) avec 9 paesi P1 strict (3 Balagne dont 2 AVF, Sartène centre, Patrimonio, 2 Alta Rocca, Bastelica, Évisa). Doctrine d'altura validée (cross-référence axe `tours_genoises` = 0 conflit commune-tour). Cas frontière arbitrés Soleil : Piana EXCLU (commune avec Tour Turghiu, malgré label AVF), Corte EXCLU (citadelle militaire hors périmètre paesi). Précédente : 14 mai 2026 (clôture série Sprint 5 archéo + castelli — 5a uniquement) — ajout 4 dettes patrimoine archéo : `SITES-PATRIMOINE-ARCHEO-ALERIA-V2-APPROFONDIE-001` (multi-composantes : forum, thermes, basilique, nécropoles), `SITES-PATRIMOINE-ARCHEO-MARIANA-V2-APPROFONDIE-001` (mithraeum, basilique paléochrétienne, baptistère, palais épiscopal), `SITES-PATRIMOINE-ARCHEO-CITES-ROMAINES-COMPLEMENT-001` (Sagona/Calvi/Centuri antiques skippées doctrine stricte), `SITES-PATRIMOINE-VILLAGES-MEDIEVAUX-DISPARUS-COMPLEMENT-001` (axe `villages_medievaux_disparus` non créé Sprint 5, candidat Saint-Jean-d'Ortolo PA2A000005 sans GPS publié). Sprint 5b et 5c vides (0 candidat passant doctrine stricte BP-SPRINT4-DOCTRINE-STRICTE-GPS-PUBLIE-001), série Sprint 5 clôturée à 5a uniquement (création 2 axes `archeo_romaine` + `castelli_oppida`, 7 migrations, ajout Mariana). Précédente : 14 mai 2026 (clôture série Sprint 4 ponts historiques + dette CSS app) — ajout 3 dettes : `SITES-PATRIMOINE-PONTS-SOUS-CIBLE-SPRINT4-001` (10/15-20 assumé scénario B), `SITES-PATRIMOINE-PONTS-SKIPS-PRECISION-GPS-001` (5 candidats archivés : Zippitoli, Zaglia, Ponti Vecchiu, Trinité, Calzola), `APP-CSS-ORPHELIN-PURGE-V8-001` (~90 lignes CSS orphelin résiduel post-purge v8, ferme formellement priorité 1 PROJECT_INSTRUCTIONS_v3.md du 22 avril 2026). Ajout 1 bonne pratique : `BP-SPRINT4-DOCTRINE-STRICTE-GPS-PUBLIE-001` (skip sans GPS publié, pas de centroïde commune en P1 pour patrimoine bâti). Précédente : 13 mai 2026 (clôture série Sprint 3 tours littorales) — ajout 7 dettes patrimoine consolidées : `SITES-PATRIMOINE-TOURS-PRE-SPRINT3-PIEVE-NULL-001` (4 tours résiduelles), `SITES-PATRIMOINE-TOURS-PRE-SPRINT3-INSEE-NULL-001` (4 tours résiduelles), `SITES-PATRIMOINE-PIEVE-ATTRIBUTIONS-SUSPECTES-001` (Meria, Olmeta, Saint-Florent), `SITES-PATRIMOINE-TOURS-CASTELLUCCIO-DOUBLET-001` (audit toponymique), `SITES-PATRIMOINE-TOURS-FARINOLE-GPS-PRECIS-001` (centroïde commune), `SITES-PATRIMOINE-TOURS-POGGIO-ERSA-MAISON-TOUR-001` (PA2B000009, GPS anomal), `SITES-PATRIMOINE-TOURS-MERIMEE-EXHAUSTIF-COMPLEMENT-001` (4 tours résiduelles 66→70). Création section `## Bonnes pratiques issues de sprints` avec 1 entrée : `BP-SPRINT3B-MAPPING-INTEGRE-INGESTION-001`. Précédente : 13 mai 2026 (Sprint 2 Phase B mégalithes + finalisation dettes) — ajout 6 dettes : `SITES-EM-JSON-UNTERMINATED-STRING-001`, `SITES-REFERENCE-JSON-DEPRECATION-001`, `REMARQUABLES-GEOLOGIQUES-DRIFT-001`, `SITES-PATRIMOINE-INSEE-BELVEDERE-CAMPOMORO-001` (anomalie A3 Cowork Phase A), `CORPUS-META-AXES-INCOMPLET-001` (anomalie A4 Cowork Phase A), `PATRIMOINE-HASH-DEEPLINK-CADRAGE-001` (cadrage map cassé sur navigation hash directe N2, identifiée pendant validation preview PR #538). Fermeture par invalidation : `SITES-PATRIMOINE-JSON-L13034-001` (audit Code H1, le fichier `sites_patrimoine.json` côté dev/main est JSON valide ; cause réelle = sandbox Cowork stale, pattern `OPS-COWORK-SANDBOX-GIT-DRIFT-001`). Précédente : 13 mai 2026 (Sprint M1/M2/M3 sites_app.json + fix patrimoine 2e clic PR #532) — ajout 3 dettes absentes du fichier : `OPS-COWORK-SANDBOX-GIT-DRIFT-001`, `SITES-PATRIMOINE-JSON-L13034-001`, `CLEANUP-PATRIMOINE-INSTRUMENTATION-001`. Aucune modification des dettes existantes. Précédente : 12 mai 2026 (Sprint micro-data cleanup post-Sprints-A/B/C) — Item 1 doublon `san_andrea` déjà accompli au Sprint B (PR #486+#487, `git mv` = rename, pas copie ; 0 fichier subsistant dans `docs/assets/visuels/san_andrea*` vérifié dans 5 worktrees actifs). Item 2 résolu : 12 slugs whitelist drifts corrigés dans `sites_patrimoine.json` + `sites_corse.json` (commit `48e3556`, illustre False→True + visuel `.png` complet). Nouvelle dette `WHITELIST-ILLUSTRE-FLAG-DRIFT-001` ouverte + résolue dans le même sprint. Pipeline `consolidate_sites.py` not run (déprécié Sprint A), édit manuel cohérent. Précédente : 12 mai 2026 (Sprint C cleanup worktrees OPS) — nettoyage des 2 résiduels mentionnés au brief : worktree root libéré (`git checkout --detach origin/main`, branche `fix/patrimoine-audit-phase-b` supprimée), worktree `hungry-austin-0b60a4` admin nettoyé (`git worktree remove --force`, branche `claude/hungry-austin-0b60a4` supprimée). Worktrees passe de 7 à 5 actifs. Dette `OPS-WORKTREE-CREATION-001` **partiellement résolue** (résiduels admin nettoyés, directory `hungry-austin-0b60a4/` orpheline sur disque comme attendu — Permission denied pendant CWD agent actif). Précédente : 12 mai 2026 (Sprint B data légers) — résolution de 2 dettes patrimoine : `ALERIA-RUINE-FICHE-V3-KEY-MISMATCH-001` (commit `db4074a`, fiche_v3_slug aligné `'aleria_ruine'` → `'aleria_antique'` dans sites_patrimoine.json + sites_corse.json) et `SAN-ANDREA-PIEDICROCE-SOURCE-INTROUVABLE-001` (nouvelle dette ouverte + résolue, commit `866cf86`, archivage des 4 visuels via `git mv` vers `_drafts/visuels_archive/` après hypothèse γ Soleil retenue — erreur exploration Cowork, recherche web exhaustive 6+ sources fiables 12 mai sans résultat). Whitelist reste à 33. Précédente : 12 mai 2026 (Sprint A pipeline réparation, option (b)) — découverte STOP A : `consolidate_sites.py` est obsolète (pas à réparer), `sites_patrimoine.json` est la SOURCE CANON runtime (et non `sites_corse.json` qui est DEPRECATED target 2026-06-05). Actions : (1) `consolidate_sites.py` déprécié explicitement (commit `7565563`, docstring + `sys.exit(2)`), (2) `sync_cross_app.py --apply` exécuté (commit `26fb064`, 28 divergences `sites_em.json` propagées depuis canonical), (3) 2 dettes reformulées/fermées : `PIPELINE-DRIFT-SITES-PATRIMOINE-001` → reformulée `CONSOLIDATE-SITES-PY-OBSOLETE-001` (basse priorité) ; `PIPELINE-DRIFT-SITES-EM-CORSE-001` → **FERMÉE** par sync_cross_app. Nouvelle dette ouverte `PIPELINE-DATA-ARCHITECTURE-DOC-001` (architecture data à documenter pour éviter re-diagnostic à chaque sprint). Précédente : 12 mai 2026 (Sprint micro-data réintégration orphelins) — réintégration de 9 sites orphelins à `sites_patrimoine.json` (commit `6094c11`, corpus Phase 1 passe 171→180) + promotion 9 slugs à `ILLUSTRATED_SPOTS` (commit `312dffe`, whitelist 24→33). Override `plateau_du_coscione` → `doyenne_extreme_sud` + `pieve_verde` dans `SPOT_DOYENNE_OVERRIDES.json`. SKIP `san_andrea_de_piedicroce` (absent canonical, brief séparé futur). **Dette `PIPELINE-DRIFT-SITES-PATRIMOINE-001` escaladée moyenne → HAUTE** (14 sites perdus cumulés en 2 mois, patches manuels itératifs). Mise à jour `ILLUSTRATED-WHITELIST-DRIFT-001` (drift 100→33 sous contrôle éditorial). `MONTE-GENOVA-NATURAL-VISUEL-001` confirmée OUVERTE (audit visuels 2026-05-12 : 0 variante `_naturel` en filesystem). Précédente : 11 mai 2026 (Sprint app-ui-polish, Items 1+2a partiels) — Item 1 ✓ : barre conditions permanente `#conditions-bar` (5 chips Kp/Réseau/Live/Orage/Contribs + accordéon 4 sections), panel `?` slim (légende EM seule). Item 2a partiel : 2 corrections GPS sites géophys (`capu_rossu` recadré sur pic 42.225/8.573, `reserve_de_scandola` aligné sur canonical 42.36072/8.56127). 2 sites skip faute source externe (`barrage_padula`, `min_argentella`) — à re-sourcer brief séparé. 2 sites confirmés OK (`calanques_de_piana` sur D81, `reserve_de_scandola` péninsule). Nouvelle dette `PIPELINE-DRIFT-SITES-EM-CORSE-001` ouverte (même cause racine que `PIPELINE-DRIFT-SITES-PATRIMOINE-001`). Items 2b (refactor popup geophys UTH-pattern) et 3 (popup calc Tellux viewport) en attente. Précédente : 11 mai 2026 (Sprint complément post-curation-N1) — Items 1/2/3 du brief précédent traités : étoile priorité unique sur `aleria_antique` (15 drops total : 13 brief + 2 drift cirque_de_bonifato/plateau_du_coscione côté `sites_corse.json`), Poggio Venaco rename + `.png` cosmétique, override `cap_corse_extreme_nord` → `doyenne_du_cap`. Item 4 (T4 régression panel aleria) classé NOMINAL après investigation chaîne `applyHash → onSpotClick → openPopup` (popup s'ouvre, pas le panel — by design, le panel s'ouvre via bouton "Ouvrir la fiche" dans la popup). Nouvelle dette `VISUEL-EXT-COSMETIC-001` ouverte (228 sites au total sans extension `.png`, sans impact runtime). `PIPELINE-DRIFT-SITES-PATRIMOINE-001` confirmée OUVERTE (toutes les modif data Items 1+2 ont dû éditer les 2 fichiers — preuve vivante du drift). Précédente : 11 mai 2026 (Sprint curation N1) — ajout de 3 nouvelles dettes ouvertes : `PIPELINE-DRIFT-SITES-PATRIMOINE-001` (drift 479 canonical → 453 publié, 22 sites perdus identité inconnue), `MONTE-GENOVA-NATURAL-VISUEL-001` (variante "sans mégalithes" à produire), `ALERIA-RUINE-FICHE-V3-KEY-MISMATCH-001` (bug latent fallback). Mise à jour `ILLUSTRATED-WHITELIST-DRIFT-001` (whitelist 12→24, drift 96→24 sous contrôle). 4 sites naturels Phase 1 (anneaux_du_cap_corse, massif_du_haut_asco, monte_d_oro, monte_renoso) ajoutés manuellement à `sites_patrimoine.json` (commit `18de84e`). Suppression des 9 fiches v3 doyennés de `fiches_patrimoine.json` (`aleria_antique` conservée, commit `7264151`). Précédente : 11 mai 2026 (Sprint dettes post-Phase-B) — résolution de 3 dettes supplémentaires : `DOYENNE-ILLUSTRATIONS-OBSOLETE-001` (S6, commit `bf3415c`, Option A commentaires actualisés), `HASH-SPOT-SEUL-001` (S7, commit `396947e`, `applyHash` étendu single-segment spot), et `SITES-COORDS-COTIERES-VERIFICATION-001` (commit `a966511`, audit GPS + commune pour `capu_di_logu`, `tour_de_capo_di_muro`, `u_paladinu`). Création sous-ticket `OPS-CODE-WORKTREE-ISOLATION-FLAG-001` (investigation cause racine `OPS-WORKTREE-CREATION-001` : worktrees auto-créés par desktop app hardcoded, remediation = usage CLI exclusif). Cleanup 7 worktrees + 6 branches orphelines (commit `c2c7ada`). Précédente : 11 mai 2026 (Sprint Phase B post-audit Cowork patrimoine.html) — résolution de 3 dettes via branche `fix/patrimoine-audit-phase-b-v2` : `SITES-NAME-NULL-001` (suppression doublon `menhirs_du_rizzanese`, commit `7f8f592`), `N2-ILLUSTRATED-SHARED-MARKER-001` (créée + résolue, fix solution A symétrique enter/exit, commit `5f1b480`), `N2-SPOT-CLICK-PROPAGATION-001` (créée + résolue, `L.DomEvent.stopPropagation` sur marker/pieve clicks, commit `4b077f4`). Création de 5 nouvelles dettes ouvertes : `HASH-SPOT-SEUL-001` (S7), `DOYENNE-ILLUSTRATIONS-OBSOLETE-001` (S6), `ILLUSTRATED-WHITELIST-DRIFT-001` (S5), `OPS-WORKTREE-CREATION-001` (14 worktrees parasites + violation doctrine), `SITES-COORDS-COTIERES-VERIFICATION-001` (3 sites lon<8.85 à investigater). Précédente : 6 mai 2026 (Brief 29 app.html migration sites_corse.json) — ajout 4 nouvelles dettes consolidation `PATRIMOINE-BASTIA-PIEVES-SOUS-ATTRIBUEES`, `SITES-CORSE-HAMEAUX-CHURCHES-INSEE-001`, `SITES-CORSE-TOPONYMES-LOCAUX-001`, `TOUR-AGNELLO-GPS-DISCORDANCE-001` (audits Cowork à programmer post-Brief 29). Statut `PATRIMOINE-ORPHANS-INVISIBLES-001` actualisé (43 sites invisibles vs 18 avant Brief 28, lié à l'élargissement de corpus et résorbable via `_drafts/SPOT_DOYENNE_OVERRIDES.json`). Précédente : 6 mai 2026 (Brief 28 patrimoine.html migration sites_corse.json) — ajout `PATRIMOINE-CLICK-CONFLICTS-001` (conflits zones de clic miniatures/polygones, identifié Brief 27) et `PATRIMOINE-LOAD-PERF-001` (3 fetches au boot, sérialisation polygones derrière sites_corse.json). Précédente : 5 mai 2026 (Brief 17 Phase A + B) — résolution `PATRIMOINE-PIEVES-39-VS-47-CASTA-001` via livraison Cowork v2 (`pieves_communes_mapping_v2_canonicite_casta.json`) : 8 pieves Casta restaurées + 39 transferts de communes appliqués par le pipeline. Sortie `pieves_polygons.json` passe de 39 à 47 pieves. Phase A préservée : `PATRIMOINE-ORPHANS-INVISIBLES-001` en voie d'éclaircissement via `_drafts/SPOT_DOYENNE_OVERRIDES.json`. Précédente : 4 mai 2026 (Brief 9 patrimoine drill-down niveau 2) — ajout de `PATRIMOINE-PIEVES-39-VS-47-CASTA-001` (périmètre 39 pieves alignées Brief 2 vs 47 Casta canoniques, 8 pieves implicitement absorbées ou sans rattachement clair). Précédente : 4 mai 2026 (Brief 8 patrimoine révisé) — actualisation des deux entrées `PATRIMOINE-TILES-ZOOM-001` et `PATRIMOINE-ORPHANS-INVISIBLES-001` pour refléter le pivot patrimoine.html sur les 10 doyennés contemporains du diocèse d'Ajaccio (vision macro Soleil 2026-05-04, drill-down 2 niveaux retiré, ~105 spots invisibles dans la nav v1). Précédente : 1ᵉʳ mai 2026 (soir, post sprints J et L) — recensement de 4 dettes nouvelles issues de l'audit Lighthouse du sprint L (`mairies.html`) : `ROBOTS-TXT-001`, `A11Y-CONTRAST-001`, `MAIRIES-CLS-TBT-001`, `MAIRIES-REDIRECTS-001`. Précédente : 1ᵉʳ mai 2026 (soir) — audit cohérence post-cycle audit Phase D : fix wording note de fermeture `WDMAM-NAMING-001` (rollback bbox-dynamique → bbox fixe par PR #190 désormais reflété correctement) ; actualisation terminologique IRSN → ASNR (anciennement IRSN) sur 2 occurrences de la dette `RADON-DATASET-COVERAGE-001` (description + condition de déblocage) selon doctrine D1bis. Précédente : 1ᵉʳ mai 2026 — fermeture EMAG-CRUSTAL-AUDIT-001 par audit (verdict : `emag` et `crustal` fonctionnellement distincts, pas de redondance, voir notes en section "Dettes fermées récemment"). Précédente : 27 avril 2026 (soir) — ajout CAPTEURS-WEB-API-001 (magnétomètre indisponible dans les navigateurs depuis Chrome M116, août 2023 ; module différé d'une éventuelle phase ultérieure couvrant une app Android native). Précédente : 27 avril 2026 — fermeture WDMAM-NAMING-001 (fusion EMAG2/WDMAM en couche unique bbox-dynamique). Précédente : 26 avril 2026 (soir) — ajout RADON-DATASET-COVERAGE-001 (couverture partielle du dataset radon 2B vs décret 2018-434, identifiée lors de l'audit préparatoire aux envois aux institutions). 26 avril 2026 — ajout des dettes CONTRIB-SCHEMA-001 (incohérence schéma stockage contributions, identifiée lors du fix Android PR #154), RADON-CLASS-DUPLICATE et HELPERS-INLINE-CONSTS (issues de la cartographie d'extraction du moteur, `docs/tellux-engine-extraction-plan.md`). 25 avril 2026 — consolidation semaine 21-25 avril : enrichissement BT-CALIBRATION-001 (priorité Haute, mesures ratios ×57 à ×210), nouvelle dette EMAG-CRUSTAL-AUDIT-001 (Cowork Session B), précisions PR # sur les fermetures ELF-CALIB-001/WMM-CROSSCHECK-001/BDFORET-V2-001/ELF-VECTOR-001/BT-ELF-001, ajout en fermées récemment de SUPABASE-COMMUNE-FIELD-001 (PR #137) et ANTENNES-REFRESH-001 (PR #138), liens démarches externes sur TÉLÉ-001/HTA-TENSION-001/RADIO-AERO-001 (lettres envoyées 28-29 avril 2026). 25 avril — ajout RTE-OPENDATA-001. 24 avril — ajout RADON-L3-UNIFICATION-001 + WDMAM-NAMING-001.
...
**Description :** `pieve_zicavo` et `pieve_patrimonio` existent en prod (`pieves_polygons.json`) mais n'ont jamais été déclarées explicitement dans le mapping amont v1/v2/v3. Elles ont été créées par des scripts ad hoc pré-QW non identifiés dans le repo. En D1, `pieve_zicavo` a reçu une composition explicite v4 par PIP best-effort (10 communes — aucun script source) ; `pieve_patrimonio` était déjà alignée (cible de transferts v3).

**Condition de déblocage :** Si le script créateur est retrouvé, comparer sa liste de communes à la composition v4. Sinon, dette à fermer après validation du rebuild PR C (la composition v4 fait foi).

---

### PATRIMOINE-B-ZONES-T2-DEFAVORABLE-001

**Statut :** Ouverte, basse priorité (décision documentée).

**Date :** 2026-05-20

**Description :** 3 sites Tier 2 défavorables restent en pin simple, sans `is_zone` : `monte_genova` et `monte_revincu` (sommets internes au polygone Agriates — sous-zones redondantes) et `anneaux_du_cap_corse` (site sous-marin, non représentable en zone sur carte standard). Décision Soleil 2026-05-18.

**Condition de déblocage :** Aucune — décision assumée. Réévaluable si une représentation carto adaptée (ex. marqueur bathymétrique) est ajoutée post-FEDER.

---

### PATRIMOINE-B-ZONES-ETANGS-CORPUS-001

**Statut :** Ouverte, basse priorité (post-FEDER, corpus Phase 2).

**Date :** 2026-05-20

**Description :** Les étangs/lagunes de la Plaine Orientale (Étang de Biguglia — RNR, Étang de Diane, Étang d'Urbino) sont absents de `sites_patrimoine.json` en tant qu'entités étendues. Vérification Phase A B-ZONES : 0 match. Ces zones humides patrimoniales seraient des candidats `is_zone` naturels (géométrie Conservatoire du Littoral ou INPN).

**Condition de déblocage :** Ajout des entités au corpus en Phase 2, puis transformation en zones polygonales (même schéma B-ZONES).

---

### PATRIMOINE-FICHES-PIEVES-VAGUE4-POST-FEDER-001

**Statut :** Ouverte, basse priorité (post-FEDER).

**Date :** 2026-05-18

**Description :** 19 pieves prod n'ont pas de fiche markdown rédigée :
- Créations Phase 1 (3) : `pieve_altiani`, `pieve_biguglia`, `pieve_castagniccia`
- Cap Corse (6) : `pieve_brando`, `pieve_canari`, `pieve_luri`, `pieve_nonza`, `pieve_pino`, `pieve_rogliano` (matière première = `cap_corse_v2/v3.md` archive corpus privé)
- Casta restaurées Brief 17 (5) : `pieve_casacconi`, `pieve_filosorma`, `pieve_mezzana`, `pieve_talcini`, `pieve_tavagna`
- Existantes sans fiche (5) : `pieve_aleria`, `pieve_celavo`, `pieve_gulfo_d_aiacciu` (rename), `pieve_patrimonio`, `pieve_zicavo`

**Condition de déblocage :** Vague rédactionnelle 4 post-FEDER, dans le corpus privé `_corpus/fiches_pieves/`.

**Note :** la `note_rattachement` dans `pieves_polygons.json` suffit pour l'UI Tellux (doctrine ADR-001). Les fiches v2/v3 sont du rédactionnel long-format, hors-périmètre runtime.

---

### PATRIMOINE-PIEVES-V7-RESHAPE-INSPECTION-001 — 11 pieves reshapées par le rebuild v7 à inspecter

**Statut :** Ouverte, basse priorité (suivi post-rebuild).

**Date :** 2026-05-21

**Description :** Le rebuild voie-a (Étape 5 PR C, `pieves_polygons.json` v7) a régénéré le dérivé prod depuis le mapping v4. 11 pieves ont une composition différente de l'ancienne prod v6 (qui était incohérente post-voie-b) : `pieve_orezza` (14→29 communes), `pieve_moriani` (5→13), `pieve_istria` (19→8), `pieve_gulfo_d_aiacciu` (10→5), `pieve_zicavo`/`pieve_mezzana`/`pieve_talcini` (+2), `pieve_bozio`/`pieve_verde` (+1), `pieve_cinarca`/`pieve_cruzini` (−1). Refonte assumée (arbitrage Soleil 2026-05-21) — la carte v4 est cohérente. Les ~26 communes ex-zombies (ampugnani/campoloro/giovellina résiduel) ont été assignées best-effort PIP en D1.

**Condition de déblocage :** Inspection visuelle des 11 pieves par Soleil sur la preview / prod. Si une forme paraît anormale, corriger le mapping v4 (`_drafts/pieves_communes_mapping_v4_cleanup_2026-05-18.json`) et relancer le rebuild voie-a (`build_pieves_polygons.py`). Aucun bug fonctionnel identifié à ce stade — dette de vérification, non de correction.
...
**Statut actualisé 2026-05-17 (audit Cowork Phase B cat 1) :** 17 des 18 orphelins résiduels retag selon géo réelle (doctrine Soleil 17 mai : géo prime sur orthodoxie historique pour Phase 1 beta). PR `fix/patrimoine-orphans-cross-doyennes`. **1 seul orphelin restant : `cap_corse_extreme_nord`** (cas légitime documenté — orphan transcommunal Cap, non rattachable à un doyenné spécifique). Dette toujours **ouverte avec périmètre résiduel = 1** en attente de la doctrine de rattachement des orphans transcommunaux (ou conservation comme orphan documenté).

**Statut antérieur 2026-05-06 (Brief 28/29) :** corpus passé de 124 → 191 sites Phase 1 (+67 nouveaux sites issus de la consolidation Cowork 4 sources). Sur les 191 sites P1, **~43 restent invisibles** post-migration : 51 sont sans `doyenne_contemporain_slug` (orphans intentionnels — diocèses abstraits, naturels transcommunaux, churches Phase 2 partiel, toponymes locaux) et l'intersection illustrés ∪ overrides ne les rapatrie pas tous. Le mécanisme d'override `_drafts/SPOT_DOYENNE_OVERRIDES.json` (Brief 17 Phase A) reste actif et la résorption progressive continue au fil des audits. La dette reste ouverte avec un nouveau périmètre 191 (vs 124).
...
**Priorité :** Faible (axe minoritaire en termes d'effectif comparé aux `patrimoine_religieux` et `megalithes`)
...
### PATRIMOINE-PIEVE-NEBBIU-CHEVAUCHE-GOLO-CAP-001 — Polygone pieve_nebbiu chevauche doyennés Golo et Cap

**Description :** Le polygone `pieve_nebbiu` (`docs/data/pieves_polygons.json`, doyenne_contemporain_majoritaire = doyenne_du_golo) couvre une zone large incluant des communes administrativement rattachées au doyenné contemporain du Cap (Saint-Florent, Patrimonio, Farinole). Conséquence : 4 sites (Mine Farinole, Cathédrale Sainte-Marie du Nebbio, San Martino de Patrimonio, Tour de Fornali) sont sur `pieve_nebbiu + doyenne_du_cap` mais le polygone est rendu côté Golo. Soleil signale ces 4 sites comme "rattachements incorrects" alors qu'ils sont géographiquement légitimes dans le Nebbio historique. Lots 4 (Golo) et lots Cap successifs ont diagnostiqué sans corriger côté data (le rattachement pieve_nebbiu est correct, c'est le polygone qui chevauche).

**Priorité :** Faible (UX visuelle uniquement, pas de bug fonctionnel data)

**Condition de déblocage :** Refonte topologique du polygone `pieve_nebbiu` — soit scinder en deux polygones distincts (Golo + Cap) avec un slug commun, soit ajuster la `doyenne_contemporain_majoritaire` selon la majorité de communes affichées. Arbitrage Soleil Phase A pour stratégie. Estimé 30-60 min Cowork avec inspection géométrique précise.

**Identifiée :** 14-15 mai 2026 (lots 4 Golo + lots Cap successifs de la série fix N3).

---

### PATRIMOINE-DETROIT-BONIFACIO-ENTREE-MANQUANTE-001 — Détroit de Bonifacio absent du data

**Description :** Soleil signale lot 10 Extrême-Sud que le "Détroit de Bonifacio a disparu" lors de l'audit visuel. Investigation Phase A PR 13 confirme : 0 entrée matchant "détroit/bonifacio/bouches" dans `sites_patrimoine.json` (hors entrées commune Bonifacio). Le Détroit n'est probablement jamais entré dans le data, soit jamais ajouté, soit supprimé par erreur antérieure. Site naturel/géographique remarquable de niveau international (Réserve naturelle des Bouches de Bonifacio, ~12 km de large, frontière maritime France-Italie).

**Priorité :** Faible (entité géographique englobante, ajout cohérent avec axes `remarquables_geologiques` ou `patrimoine_naturel_sacre`, mais ne bloque pas la prod)

**Condition de déblocage :** PR ajout dédiée nécessitant décisions sur GPS de référence (centre détroit ~41.38/9.20), axe `axe_corpus`, sources institutionnelles (Réserve naturelle, IGN), et pieve cible (probablement `pieve_bonifacio`). Estimé 30 min Cowork avec arbitrage Soleil sur formulation.

**Identifiée :** 15 mai 2026 (PR 13 lot 10 Extrême-Sud Phase A).

---

### PATRIMOINE-PIEVE-CAP-NON-CLIQUABLE-UI-001 — Pieve sud-ouest du Cap non cliquable (bug UI non reproductible programmatiquement)

**Description :** Soleil signale dès le lot 3 (PR 6) qu'une pieve située au sud-ouest du Cap n'est pas cliquable en N3. Investigation PR 6 + lot 12 PR 15 : tests programmatiques sur les 7 pieves polygones du Cap (`pieve_bastia`, `pieve_brando`, `pieve_canari`, `pieve_nonza`, `pieve_luri`, `pieve_pino`, `pieve_rogliano`) montrent que toutes ont `pointerEvents=auto`, `hasClickHandler=true`, `tlxPieveSlug` défini, et transitionnent correctement en N3 avec les bons sites. **Le bug ne se reproduit pas en click programmatique** — probable cause : bug visuel z-order/overlap (un polygone par-dessus un autre, l'utilisateur clique sur le mauvais), ou polygone avec contour très fin difficile à cliquer.

**Priorité :** Faible (UX uniquement, contournable via deeplink hash `#du_cap/<pieve>`)

**Condition de déblocage :** Reproduction nécessaire avec test utilisateur réel (souris physique vs simulation programmatique) pour identifier le polygone précis. Une fois identifié : soit ajustement z-order Leaflet (`bringToFront/Back`), soit épaississement contour interactif, soit séparation polygones chevauchants. Estimé 30 min debug session live avec Soleil.

**Identifiée :** 14-15 mai 2026 (lot 3 Cap PR 6 puis lot 12 PR 15).
...
### SITES-PATRIMOINE-PONTS-SOUS-CIBLE-SPRINT4-001 — Sprint 4 sous-cible 10/15-20 assumée

**Description :** Série Sprint 4 ponts historiques (4a Haute-Corse + 4b CS-ouest + 4c CS-sud) clôturée à **10 ponts sur cible révisée 15-20** (scénario B couverture représentative décidé Soleil post-Sprint 4a). Représente 67% de la cible basse, 50% de la cible haute. Cible brief initial Sprint 4 était 40 ponts, manifestement surévaluée vs réalité de Mérimée POP exposant ces ponts avec GPS fiables. Distribution finale : 5 ponts Phase 1 strict (Mérimée + GPS publié) + 5 Phase 2 latent (GPS approximatif lieu-dit ou centroïde commune sur les IA inventaire). Bonne couverture représentative géographique : Haute-Corse Sprint 4a (8 ponts dont Castagniccia, Asco, Niolu, Tavignano, Golo), CS-ouest Sprint 4b (Pianella Ota), CS-sud Sprint 4c (Spina-Cavallu Sartène).

**Priorité :** Faible (Phase 1 acceptable à 10 ponts représentatifs)

**Condition de déblocage :** Si dossier FEDER OS1.2 ou besoin patrimoine étendu exige cible >10 ponts, sprint complémentaire Cowork Mérimée POP exhaustif dédié (filtres + scrolling complet base Mérimée Corse, hors Wikipedia FR seul). Estimation 4-6h. Référence pratique : pattern Cowork mapping Sprint 3a, mais en mode exploration sources au lieu de mapping enrichissement.

**Décision Soleil 14 mai 2026** : pas d'action immédiate, couverture représentative suffisante pour Phase 1.

**Identifiée :** 14 mai 2026 (clôture série Sprint 4 ponts historiques).

---

### SITES-PATRIMOINE-PONTS-SKIPS-PRECISION-GPS-001 — 5 candidats ponts skippés Sprint 4b/4c (précision GPS différée)

**Description :** Sprint 4b a skippé 4 candidats CS-ouest pour absence de GPS publié fiable :
- `pont_genois_de_zippitoli_bastelica` (Bastelica, Prunelli, PA00099075, XVᵉ-XVIᵉ)
- `pont_de_zaglia_evisa` (Evisa, Aitone, PA00099090, XVIIIᵉ)
- `ponti_vecchiu_taravo_zigliara` (Zigliara/Petreto-Bicchisano, Taravo, PA00099126 + PA00099101, XVᵉ)
- `pont_de_la_trinite_olivese` (Olivese, Taravo haut, sans Mérimée PA, 1698)

Sprint 4c a skippé 1 candidat CS-sud :
- `pont_de_calzola_pila_canale` (Pila-Canale/Casalabriva, Taravo aval, recensé Mérimée 1839 mais pas de PA explicite, XVIᵉ)

Ces 5 ponts sont identifiés Mérimée (ou Mérimée 1839 pour Calzola) mais leurs coordonnées GPS publiées sont absentes, imprécises, ou non cross-vérifiables sous le seuil de temps doctrinal Sprint 4 (~30 min par candidat). Discipline P1 strict respectée Sprint 4b/4c (cf. apprentissage Sprint 4b/4c doctrine GPS publié, BP-SPRINT4-DOCTRINE-STRICTE-GPS-PUBLIE-001).

**Priorité :** Faible (5 ponts patrimoine intéressant mais non bloquants Phase 1)

**Condition de déblocage :** Sprint précision GPS dédié, dimensionnement ~2-3h pour les 5. Méthodes possibles : consultation Mérimée fiches détaillées (champ "Référence cadastrale" + reconstruction GPS), extraction Wikipedia FR fiches dédiées si elles existent, ou cartographie IGN avec lecture coordonnées d'un repère identifié sur la fiche Mérimée. Si déblocage : ajout P1 ou P2 latent selon précision finale, dans l'axe `ponts_historiques` existant.

**Identifiée :** 14 mai 2026 (clôture série Sprint 4 ponts historiques).

---

### APP-CSS-ORPHELIN-PURGE-V8-001 — CSS orphelin résiduel post-purge v8 d'`app.html` (~90 lignes)

**Description :** Audit chat session 14 mai 2026 a confirmé que la purge v8 d'`app.html` (résidus patrimoine + agronomie) est **substantiellement faite** par rapport au brief `PROJECT_INSTRUCTIONS_v3.md` du 22 avril 2026 :
- Titre HTML EM-pur ✅
- Biblio Thom/Leplat/Hoskin retirée ✅
- Panneau hypothèses HTML commenté ✅
- Bouton FAB Quête commenté ✅
- Layers patrimoine migrées vers `patrimoine.html` ✅
- Layers agronomie migrées vers `agronomie.html` ✅

**Résidus restants** : CSS orphelin (sélecteurs jamais matchés par le HTML actuel) :
- Bloc `.quest-btn`, `.quest-tooltip`, `.quest-badge` (~30 lignes)
- Bloc `.hypo-panel*` complet (header / body / foot / nav / close / counter / id / title / desc / meta) (~40 lignes)
- Media query mobile `.quest-btn` + `.hypo-panel` (~5 lignes)
- Animation `@keyframes badgePulse` (~3 lignes)

Total estimé : **~80-90 lignes CSS pur**, zéro impact fonctionnel runtime (les classes ne sont jamais utilisées dans le HTML actif). Pas de risque, mais cohérence sémantique.

**Priorité :** Faible (cosmétique, pas de symptôme runtime)

**Condition de déblocage :** À clôturer lors d'une prochaine modification d'`app.html` pour autre raison (refactor CSS, ajout fonctionnalité EM, etc.). Travail estimé < 30 min. Pas urgent.

**Note formelle :** ferme la priorité 1 « chantier de purge » de `PROJECT_INSTRUCTIONS_v3.md` (22 avril 2026) comme « substantiellement résolue par v8 + reliquat CSS tracé ». L'instruction v3.md peut être considérée comme acquittée pour cette priorité 1.

**Identifiée :** 14 mai 2026 (audit chat session post-clôture Sprint 4 ponts).

---

### SITES-PATRIMOINE-ARCHEO-ALERIA-V2-APPROFONDIE-001 — Aleria multi-composantes v2 approfondie

**Description :** Doctrine Sprint 5 validée Soleil : Aleria = **1 site unique** dans le corpus (`aleria_antique`, axe `archeo_romaine` post-Sprint 5a, GPS 42.1142/9.5131, gps_locked, étoile priorité, visuel `_tellux_v2`). Cette représentation unique masque la richesse multi-composantes de la cité antique : forum, thermes, basilique paléochrétienne, palais épiscopal, nécropoles préromaine et romaine, port antique, théâtre, prétoire. Une représentation v2 multi-composantes (un site parent + N sous-sites positionnés précisément) serait nécessaire pour exploitation patrimoniale fine (parcours, fiches v3 par composante, audio-guide, etc.).

**Priorité :** Faible (représentation actuelle suffisante Phase 1)

**Condition de déblocage :** Sprint dédié multi-composantes Aleria, en coordination avec Musée d'archéologie d'Aleria. Méthodologie : fouille des plans de site officiels (ISULA Patrimoine, Service Régional d'Archéologie), géolocalisation parcellaire de chaque composante, conception schéma data parent/enfant ou hiérarchique. Estimation 1-2 jours pour Aleria seule. Préfigure pattern v2 réutilisable Sprint Mariana et autres cités archéo majeures.

**Identifiée :** 14 mai 2026 (Sprint 5a Phase A doctrine Soleil).

---

### SITES-PATRIMOINE-ARCHEO-MARIANA-V2-APPROFONDIE-001 — Mariana multi-composantes v2 approfondie

**Description :** Doctrine Sprint 5 validée Soleil : Mariana = **1 site unique** dans le corpus (`mariana_antique`, axe `archeo_romaine` créé Sprint 5a, GPS publié 42.53928/9.49597, Mérimée PA00099208). Cette représentation unique masque la richesse multi-composantes de la cité romaine de Marius : mithraeum (découvert INRAP 2017, premier de Corse), basilique paléochrétienne, baptistère hexagonal, palais épiscopal, nécropoles, rempart trapézoïdal délimitant ~30 ha. Représentation v2 multi-composantes nécessaire pour exploitation patrimoniale fine.

**Priorité :** Faible (représentation actuelle suffisante Phase 1)

**Condition de déblocage :** Idem `SITES-PATRIMOINE-ARCHEO-ALERIA-V2-APPROFONDIE-001`. Coordination avec Musée archéologique de Mariana (Lucciana) et INRAP. Pattern hérité Aleria v2 si fait avant.

**Identifiée :** 14 mai 2026 (Sprint 5a Phase A doctrine Soleil).

---

### SITES-PATRIMOINE-ARCHEO-CITES-ROMAINES-COMPLEMENT-001 — Cités antiques romaines secondaires non ingérées Sprint 5b

**Description :** Sprint 5b ciblait l'ajout de cités antiques romaines secondaires identifiées en Sprint 5a Phase A : Sagona antique (Vapone/Vicones), Calvi antique (Sinus Casalus), Centuri antique (Centurinum Civitas). Recherche Wikipedia FR + Mérimée POP a confirmé que **aucune ne possède de fiche site archéo distincte** publiée avec GPS exploitable :
- Sagona : Sant'Appiano (cathédrale romane, déjà couverte axe `patrimoine_religieux`) seul site documenté ; pas de site antique propre référencé Mérimée avec GPS
- Calvi : occupation diffuse multiples lieux-dits (Alga, Raccu, Revellata, Serra, Agnellu Grotto), pas de Mérimée site antique distinct
- Centuri : traces fragmentaires lieux-dits Civita/Palombese, pas de Mérimée

Doctrine `BP-SPRINT4-DOCTRINE-STRICTE-GPS-PUBLIE-001` appliquée → Sprint 5b clôturé sans PR (0 candidat ingérable).

**Priorité :** Faible (cités antiques secondaires, faible visibilité touristique vs Aleria/Mariana)

**Condition de déblocage :** Sprint précision archéo dédié (si dossier FEDER OS1.2 ou besoin patrimoine étendu) — méthodes possibles : fouilles INRAP rapports, Service Régional d'Archéologie bilans, articles MEFRA OpenEdition, cartographie IGN avec lecture GPS lieu-dit. Estimation ~3-5h pour les 3 cités.

**Identifiée :** 14 mai 2026 (Sprint 5b Phase A doctrine stricte appliquée).

---

### SITES-PATRIMOINE-VILLAGES-MEDIEVAUX-DISPARUS-COMPLEMENT-001 — Axe `villages_medievaux_disparus` non créé + candidat Saint-Jean-d'Ortolo

**Description :** Sprint 5c ciblait la création de l'axe `villages_medievaux_disparus` + ingestion de paesi désertés pré-génois. Recherche Wikipedia FR + Mérimée POP a confirmé un candidat majeur : **Village médiéval dit de Saint-Jean-d'Ortolo (Sartène)** — Mérimée PA2A000005, MH 2009, XVe-XVIe, ~28 maisons + 2 tours défensives, abandonné premier quart XVIᵉ. Toutefois **GPS non publié** (uniquement indication "vallée Ortolo, alt 300m, contreforts Cagna"). Doctrine `BP-SPRINT4-DOCTRINE-STRICTE-GPS-PUBLIE-001` appliquée → SKIP.

Conséquence : axe `villages_medievaux_disparus` **non créé Sprint 5** (0 candidat passant doctrine stricte). Création différée jusqu'à identification d'au moins 1 candidat avec GPS publié.

**Priorité :** Faible

**Condition de déblocage :** Sprint précision GPS dédié pour Saint-Jean-d'Ortolo (méthodes : article Sartène ADLFI OpenEdition, cartographie IGN vallée Ortolo, plan de site SRA). Élargissement recherche autres villages désertés Mérimée (IA inventaire général, paesi de Patrimonio ancien, etc.). Si ≥1 candidat avec GPS publié → création axe `villages_medievaux_disparus` (couleur DA proposée Sprint 5a Phase A : `#3F5B3A` Maquis ou `#A89B8C` Pierre claire).

**Identifiée :** 14 mai 2026 (Sprint 5c Phase A doctrine stricte appliquée).

---

### SITES-PATRIMOINE-PAESI-D-ALTURA-COMPLEMENT-001 — Sprint 6 sous-cible 9/25 assumée

**Description :** Sprint 6 paesi d'altura clôturé à **9 paesi sur cible 25** (36% de cible brief). Scénario B « couverture représentative > exhaustivité » appliqué (même doctrine que Sprint 4 ponts post-#553, et `BP-SPRINT4-DOCTRINE-STRICTE-GPS-PUBLIE-001`).

Cause principale de la sous-cible :
- **Doctrine d'altura filtrante** : exclusion automatique des paesi associés à une tour génoise littorale (cohérent culturellement avec la définition « paese non dépendant historiquement d'une défense côtière », mais ampute mécaniquement les paesi importants type Erbalunga, Centuri, Macinaggio, Saint-Florent, et le cas frontière Piana — exclu malgré son label AVF national)
- **Critère sources publiées strict** : seuls les paesi documentés Mérimée + AVF + PCC + Wikipedia FR (Wikidata QID) retenus en P1 strict. La liste PCC officielle Corse 2026 ne contient que Calvi et L'Île-Rousse, toutes deux exclues par doctrine d'altura → 0 PCC ingérable
- **Cas frontière Soleil arbitrés EXCLU** : Piana (AVF mais commune-tour Turghiu) + Corte (citadelle militaire hors périmètre paesi)

Couverture obtenue (9 paesi, P1 strict) : 3 Balagne (Sant'Antonino + Pigna AVF, Speloncato sans label) + Sartène centre + Patrimonio + 2 Alta Rocca (Aullène, Levie) + Bastelica + Évisa.

**Priorité :** Faible (Phase 1 acceptable à 9 paesi représentatifs)

**Condition de déblocage :** Sprint complémentaire Cowork possible avec :
- **Élargissement sources** : ouvrir aux mentions Wikipedia FR article dédié (sans Wikidata QID strict) — paesi reconnus régionalement mais sans label national
- **Label régional** : intégration label CDC Corse « Paese di Caratteru » si publié officiellement (à vérifier disponibilité 2026)
- **Audit cas frontière re-ouvert** : ré-arbitrage Bonifacio (paese-citadelle Haute Ville) / Cargèse (intérieur grec d'altura) / Calvi (citadelle militaire avec PCC label) / Cervione (versant Castagniccia côte est) / Nonza (paese-falaise Cap Corse) selon doctrine d'altura assouplie ou ré-affinée (paese-citadelle d'altura avec tours secondaires sur même commune)

**Décision Soleil 14 mai 2026** : pas d'action immédiate, 9/25 suffisant pour Phase 1. Cohérence avec doctrine Sprint 4 (10/15-20 ponts assumés).

**Identifiée :** 14 mai 2026 (clôture Sprint 6 paesi d'altura).

---

### SITES-PATRIMOINE-PIEVES-SOUS-REPRESENTEES-COMPLEMENT-001 — 7 pieves sous-représentées restantes post-Sprint 7

**Description :** Pré-audit cartographique 14 mai 2026 a identifié 8 pieves canoniques avec < 5 sites patrimoine (sous-représentation N2 cible 5-10). Sprint 7 densification ciblée (Option B Soleil approfondi) a appliqué doctrine stricte `BP-SPRINT4-DOCTRINE-STRICTE-GPS-PUBLIE-001` et identifié **1 seul candidat ingérable** (Pont de Muricciolu, Albertacce, pieve_niolu — sortie de la liste).

**7 pieves restent sous-représentées** (< 5 sites patrimoine) :
- `pieve_vivario` (1 site) : Vivario/Bocognano, Corse intérieure
- `pieve_vallerustie` (3) : Cambia, Bozio nord
- `pieve_filosorma` (3) : Galéria-Manso intérieur
- `pieve_ampugnani` (3) : La Porta/Campile, Castagniccia
- `pieve_ghisoni` (4) : Ghisoni, Fium'Orbo haut
- `pieve_tavagna` (4) : Talasani/Pero-Casevecchie/Velone-Orneto, Tavagna côte est
- `pieve_giovellina` (4) : Castellare-di-Mercurio/Erbajolo/Casanova/Canavaggia, Cortenais

Recherches Mérimée POP + Wikipedia FR effectuées Sprint 7 (Option B) sur ces pieves n'ont pas révélé de candidats avec GPS publié strict (édifices romans supplémentaires, ponts génois locaux, castelli). Le seul résultat publié solide identifié (Pont de Muricciolu pieve_niolu) a déjà été ingéré.

**Priorité :** Faible (sous-représentation N2, pas de blocage Phase 1)

**Condition de déblocage :** Sprint Cowork dédié exploration patrimoine local + sources moins centrales :
- Mérimée POP exhaustif filtre par commune
- Inventaire Général Service Régional d'Archéologie (SRA Corse)
- Médiathèque Culturelle Corse (m3c.universita.corsica)
- Cartographie IGN avec lecture lieux-dits historiques
- Articles locaux Castagniccia / Niolu / Boziu (revues régionales)

Estimation : 4-6h Cowork pour les 7 pieves, avec critère possiblement assoupli (GPS approximatif lieu-dit acceptable si Mérimée IA ou source institutionnelle SRA disponible).

**Identifiée :** 14 mai 2026 (Sprint 7 Option B doctrine stricte appliquée).

---

### SITES-PATRIMOINE-COMMUNES-MULTI-DOYENNES-EGALITE-001 — 15 communes en égalité parfaite multi-doyennés

**Description :** Sprint correction rebalance doyenne_contemporain_slug du 14 mai 2026 (PR #569) a aligné 23 sites + 1 fix manuel sur la majorité claire (≥2:1) de leur commune. Cependant **15 communes** présentaient une égalité parfaite (1-1, 2-2 ou triple) empêchant la décision automatique :

- **Égalité 2-2** : Cervione (du_golo 2 / plaine_orientale 2), Coti-Chiavari (prunelli_taravo_valinco 2 / ajaccio 2)
- **Égalité 1-1** : Altiani (cortenais 1 / du_golo 1), Stazzona (du_golo 1 / extreme_sud 1), Piedicorte-di-Gaggio (du_golo 1 / cortenais 1), Castifao (cortenais 1 / balagne 1), Serra-di-Scopamène (extreme_sud 1 / plaine_orientale 1), Piazzole (cortenais 1 / du_golo 1), Tavera (piana_vico_sari 1 / ajaccio 1), Pianello (cortenais 1 / prunelli_taravo_valinco 1), San-Giuliano (plaine_orientale 1 / du_golo 1), Bocognano (piana_vico_sari 1 / cortenais 1), Penta-di-Casinca (plaine_orientale 1 / du_golo 1), Viggianello (extreme_sud 1 / prunelli_taravo_valinco 1)
- **Multi-doyennés sans majorité** : **Monte** (du_golo 2 / cortenais 2 / balagne 1 / plaine_orientale 1) — 4 doyennés différents

Ces communes sont à la frontière entre 2-4 doyennés contemporains modernes ; leur rattachement nécessite arbitrage géographique/diocésain manuel.

**Priorité :** Faible (incohérence mineure, n'empêche pas le rendu)

**Condition de déblocage :** Arbitrage manuel Soleil par commune sur la base de :
- Carte officielle des 9 doyennés du diocèse contemporain d'Ajaccio
- Localisation précise du chef-lieu et hameaux principaux de chaque commune
- Cohérence avec rattachements existants pour communes voisines

Mini-PR data ciblée correction si arbitrage effectué. ~15-30 min selon nombre de cas tranchés.

**Identifiée :** 14 mai 2026 (sprint correction rebalance Option B Soleil PR #569).

---

### PATRIMOINE-PIEVE-MARIANA-MEGA-FUSION-001 — `pieve_mariana` mega-pieve héritage fusions caccia→giovellina→mariana

**Description :** `pieve_mariana` est un mega-pieve résultat des fusions cumulatives :
1. `merge_caccia_giovellina.py` (caccia Cortenais → giovellina Cortenais)
2. `merge_giovellina_mariana.py` (giovellina → mariana)

Conséquences : (1) polygone pieve_mariana spanne géographiquement 3+ doyennés contemporains (cortenais, golo, balagne) ; (2) `doyennes_visibles = ['doyenne_balagne', 'doyenne_du_golo']` ne contient PAS `doyenne_cortenais` alors que `doyenne_contemporain_majoritaire = doyenne_cortenais` → pieve invisible en N2 Cortenais (son doyenné de rattachement principal), visible en N2 Golo/Balagne ; (3) 10 sites pieve_mariana ont une géo doyenné différente du majoritaire (in_pv=True), laissés en l'état par PR `fix/patrimoine-orphans-cross-doyennes` selon arbitrage Soleil 17 mai (refactor passe par Cowork) ; (4) le nom "Mariana" ne reflète plus le contenu effectif (Mariana antique est côtière près Bastia, le mega-pieve inclut Asco/Cinto haute montagne).

**Priorité :** Moyenne (impact UX navigation patrimoine N3, mais pas bloquant)

**Condition de déblocage :** Audit doctrinal Cowork + ADR dédié. Options envisagées :
- (A) Renommer pieve_mariana vers nom géo cohérent (ex `pieve_castagniccia` ou `pieve_orezza_caccia`) + ajuster `doyennes_visibles` pour inclure cortenais
- (B) Annuler la fusion giovellina→mariana, recréer pieve_caccia avec son périmètre historique
- (C) Splitter en sous-pieves selon doyennés contemporains majoritaires (3 pieves issues)
...
**Statut 2026-05-18 (Stratégie D Phase 1) : RÉSOLU.** `pieve_mariana` splittée en 3 :
- `pieve_biguglia` (11 communes — Bigorno, Biguglia, Borgo, Campitello, Canavaggia, Furiani, Lento, Lucciana, Scolca, Vignale, Volpajola) → doyenne_du_golo
- `pieve_balagne` (+2 communes : Novella, Pietralba) → doyenne_balagne
- `pieve_castagniccia` (7 communes nord-Cortenais : Asco, Castifao, Castiglione, Moltifao, Piedigriggio, Popolasca, Prato-di-Giovellina) → doyenne_cortenais — rename de pieve_mariana
13 sites retagués selon migration + 13 ghost rescue PIP géo (sites taggés pieve_mariana hors brief 28). Alias `pieve_mariana → pieve_castagniccia` ajouté `pieve_aliases.json` v2. Dette fermée.
...
### PATRIMOINE-PIEVE-BIGUGLIA-NEW-001 — Pieve biguglia nouvellement créée (Stratégie D Phase 1)

**Description :** `pieve_biguglia` créée 2026-05-18 (Stratégie D Phase 1) par split `pieve_mariana`. 11 communes (Bigorno, Biguglia, Borgo, Campitello, Canavaggia, Furiani, Lento, Lucciana, Scolca, Vignale, Volpajola) → doyenne_du_golo. Diocèse médiéval Mariana conservé (Mariana antique = Lucciana est dans cette pieve).

**Priorité :** Faible (créée et fonctionnelle ; enrichissement post-FEDER).

**Condition de déblocage :** Post-FEDER : enrichir sources historiques + polygone affiné si besoin + `note_rattachement` pédagogique pour preuve UX.

**Identifiée :** 18 mai 2026 (Stratégie D Phase 1).

---

### PATRIMOINE-PIEVE-ALTIANI-NEW-001 — Pieve altiani nouvellement créée (Stratégie D Phase 1)

**Description :** `pieve_altiani` créée 2026-05-18 (Stratégie D Phase 1) par split `pieve_rogna`. 3 communes (Altiani, Piedicorte-di-Gaggio, Pietraserena) → doyenne_cortenais. Vezzani 2B347 et Pietroso 2B229 non inclus (arbitrage Soleil — déjà cohérents dans pieve_venaco).

**Priorité :** Faible.

**Condition de déblocage :** Post-FEDER : enrichir, éventuellement absorber Vezzani 2B347 si contiguité confirme. Idem `note_rattachement` UX.

**Identifiée :** 18 mai 2026 (Stratégie D Phase 1).

---

### PATRIMOINE-PIEVE-CASTAGNICCIA-RENAME-001 — Choix toponymique castagniccia à reconsidérer

**Description :** `pieve_mariana` renommée `pieve_castagniccia` 2026-05-18 (Stratégie D Phase 1, 7 communes restantes après split : Asco, Castifao, Castiglione, Moltifao, Piedigriggio, Popolasca, Prato-di-Giovellina). Le nom "Castagniccia" est imparfait toponymiquement (ces 7 communes sont plutôt dans le bassin Asco/Caccia, pas en Castagniccia historique) mais arbitrage Soleil acté ; réévaluation post-FEDER possible.

**Priorité :** Faible (UX acceptable Phase 1 beta, doctrine ADR-001 atlas pédagogique).

**Condition de déblocage :** Post-FEDER : audit toponymique strict, rename éventuel vers `pieve_caccia` (re-création) ou `pieve_asco_caccia` ou autre.

**Identifiée :** 18 mai 2026 (Stratégie D Phase 1).

---

### PATRIMOINE-PIEVE-VEZZANI-PIETROSO-2B347-2B229-001 — Décision Vezzani/Pietroso conservation pieve_venaco

**Description :** Vezzani 2B347 et Pietroso 2B229 maintenus dans `pieve_venaco` (déjà cohérents doyenne_cortenais). Vezzani contiguë cluster altiani (3.7 km, touches=True) mais arbitrage Soleil 2026-05-17 = non inclus (variante minimale Stratégie D Phase 1).

**Priorité :** Faible.

**Condition de déblocage :** Post-FEDER : réévaluation incluant `pieve_altiani` éventuellement enrichie.

**Identifiée :** 18 mai 2026 (Stratégie D Phase 1, arbitrage Soleil Q2).

---

### CORPUS-OPHI-PREFIXE-001 — Préfixe de slug `ophi_*` trompeur à proscrire

**Statut :** Ouverte, basse priorité (cosmétique).

**Date :** 2026-05-21

**Description :** Le préfixe de slug `ophi_*` (ophiolite) a été utilisé pour des entrées désignant en réalité des mines, pas des affleurements géologiques — cas `ophi_farinole`, qui s'est avéré un doublon de `mine_de_magnetite_de_farinole` (cf. `CORPUS-DOUBLONS-FARINOLE-001`). Le seul porteur du préfixe a disparu avec le merge Farinole en vague 2 : aucune entrée `ophi_*` résiduelle dans `sites_corse.json`. Une passe cosmétique est donc sans objet aujourd'hui.

**Condition de déblocage :** Aucune action requise tant qu'aucune entrée `ophi_*` n'est réintroduite. Convention pour les futurs ajouts : réserver `ophi_*` aux affleurements ophiolitiques sans exploitation, utiliser `mine_*` pour les sites miniers.

---

### OLMETA-CAP-GPS-AFFINAGE-001 — GPS du gisement d'amiante d'Olmeta-du-Cap à affiner

**Statut :** Ouverte, priorité moyenne.

**Date :** 2026-05-21

**Description :** Le site `mine_amiante_olmeta_du_cap` ajouté en vague 2 a un GPS provisoire (42.7665, 9.3771) = centroïde de la commune Olmeta-di-Capocorso (2B187). Le gisement réel est sur le versant ouest du Cap Corse, en bordure nord avec Canari. `gps_source` = « BRGM 80-RDM-003-FE (centroïde commune, à affiner) », `description_em.precision_coord` = « secteur ».

**Condition de déblocage :** Vague 3 — recaler le GPS via la carte géologique BRGM Cap Corse 1/50000, ou toute source documentant la position des sondages de 1958 (compagnie de Pont-à-Mousson).

---

### OLMETA-CAP-PIEVE-MAPPING-001 — Pieve du gisement d'Olmeta-du-Cap à confirmer

**Statut :** Ouverte, basse priorité.

**Date :** 2026-05-21

**Description :** Le site `mine_amiante_olmeta_du_cap` (commune Olmeta-di-Capocorso, INSEE 2B187) a reçu `pieve_slug = pieve_nonza` en vague 2. Justification : le mapping commune→pieve canonique (v1, inchangé en v2/v3/v4) rattache 2B187 à `pieve_nonza`, et les 2 sites 2B187 préexistants du corpus (`san_pietro_olmeta_di_capocorso`, `san_cesario_d_olmeta_olmeta_di_capocorso`) portent eux aussi `pieve_nonza`. `_drafts/FARINOLE_DECISION.json` proposait `pieve_canari` en supposant un mapping « v4 cleanup » — supposition non confirmée (le v4 ne mentionne pas 2B187).

**Condition de déblocage :** Vague 3 — si le recalage GPS (cf. `OLMETA-CAP-GPS-AFFINAGE-001`) place le gisement en limite immédiate de la pieve de Canari, réévaluer le rattachement ; sinon `pieve_nonza` fait foi et la dette peut être fermée.

---

### OLMETA-CAP-VISUEL-001 — Visuel à produire pour le gisement d'Olmeta-du-Cap

**Statut :** Ouverte, basse priorité.

**Date :** 2026-05-21

**Description :** Le site `mine_amiante_olmeta_du_cap` ajouté en vague 2 n'a pas de visuel (`visuel: null`, `illustre: false`). Il n'apparaît donc pas dans la sélection illustrée et reste écarté du filtre top 8.

**Condition de déblocage :** Production d'un visuel DA v2 pour ce site, puis passage `illustre: true` + renseignement du champ `visuel` (et `ILLUSTRATED_SPOTS` si pertinent).
...
### BP-SPRINT4-DOCTRINE-STRICTE-GPS-PUBLIE-001 — Skip sans GPS publié, pas de centroïde commune en P1 pour patrimoine bâti

**Contexte initial :** Sprint 4a Phase A a identifié 8 ponts ingérables avec GPS exploitable (3 P1 strict + 5 P2 latent IA inventaire général centroïde commune). Sprint 4b Phase A a identifié 5 candidats CS-ouest dont 4 sans GPS publié précis (Zippitoli, Zaglia, Ponti Vecchiu, Trinité). Tentation initiale : promouvoir centroïde commune en P2 latent comme on l'avait fait pour mégalithes Sprint 2 (Casalabriva, Tremica, Vasculaghju).

**Décision doctrinale Sprint 4b (arbitrée Soleil 14 mai 2026) :** la justification "patrimoine intéressant mais GPS non garanti" ne suffit pas pour les ponts. Contrairement aux mégalithes (objets ponctuels parfois enfouis dans la nature), un pont historique est par définition visible et géolocalisable précisément dans des sources publiées (Mérimée fiche détaillée, Wikipedia FR fiche dédiée, IGN). Si aucune source publiée ne donne le GPS → c'est un signal de fragilité de la documentation accessible, pas un cas de "GPS approximatif légitime". Application : Sprint 4b a skippé 4 candidats, ingéré 1 seul (Pianella Ota, GPS publié). Sprint 4c a skippé 1 candidat (Calzola), ingéré 1 seul (Spina-Cavallu Sartène).

**Pattern à appliquer aux sprints futurs touchant patrimoine bâti** (édifices romans, paesi, vernaculaire, tours, ponts) :
1. Si GPS introuvable dans Mérimée + Wikipedia FR + sources institutionnelles en <30 min → **skip** et archive en dette précision GPS différée
2. **Ne pas** approximer par centroïde commune sauf cas exceptionnel (objet diffus, complexe archéologique étendu, ou cas mégalithique)
3. La doctrine Sprint 2 mégalithes (P2 latent centroïde commune) reste acceptée pour les objets mégalithiques (cohérent avec leur nature souvent enfouie/peu documentée), pas pour le patrimoine bâti standard.
4. Coexistence avec BP-SPRINT3B : on peuple toujours `pieve_slug` + `commune_insee` dès l'ingestion. La doctrine GPS publié strict s'applique en complément pour le critère P1 strict.

**Applicable à :** Sprint 5 archéo (sauf castelli/oppida diffus), Sprint 6 paesi (P2 acceptable pour paesi entiers vu leur nature territoriale), Sprint 7 vernaculaire (skip strict si GPS introuvable), sprints complémentaires.

**Précédent acceptant centroïde commune (mégalithes, à ne pas généraliser)** : Sprint 2 Phase B (Casalabriva Contra Maio, Tremica Casaglione, Vasculaghju Figari).

**Origine :** Sprint 4b (situation problème, skip 4 candidats, PR #555), Sprint 4c (maintenu, PR #557, skip 1 candidat).

---

### BP-FIX-RATTACHEMENT-COMPLET-001 — Cohérence pieve_slug + doyenne_contemporain_slug dans toute PR rattachement

**Contexte initial :** PR #585 (rattachements lot 1 série fix N3, 14 mai 2026) a corrigé 8 sites en modifiant `doyenne_contemporain_slug` seul (pour 4 d'entre eux) ou `pieve_slug` seul (pour les autres). Conséquence : les 4 sites Cap (Tour Fornali, Cathédrale du Nebbio, San Martino de Patrimonio, Mine Farinole) sont apparus dans le bon doyenné côté N1 (`doyenne_du_cap`) mais leur `pieve_slug` les rattachait à `pieve_nebbiu` (polygone majoritaire Golo). Le drill-down N3 par pieve continuait à les afficher dans le mauvais contexte visuel. Lots 4, 8, 9 ont dû compléter ce travail (PRs #591, #599, #601) pour corriger le `pieve_slug` résiduel.

**Pattern observé sur la série fix N3 (14-15 mai 2026)** :
- PR #585 lot 1 : 8 sites corrigés sur `doyenne_contemporain_slug` ET `pieve_slug` (cohérent)
- 4 sites Cap déjà sur `doyenne_du_cap` (PR antérieure `eb09064` avait corrigé doyenné seul) mais `pieve_slug = pieve_nebbiu` à compléter — diagnostiqué lot 4 PR #591
- Lot 8 Ajaccio (PR #599) : 2 sites avec doyenné OK mais pieve héritée d'Ajaccio à corriger
- Lot 9 PTV (PR #601) : Bastelica paese pieve résiduelle pieve_celavo après doyenné corrigé

**Doctrine** :
- Avant d'écrire une correction de rattachement géographique d'un site, **identifier la pieve cible** (pas seulement le doyenné cible)
- Le champ `pieve_slug` est l'ancre architecturalement la plus forte pour le N3 drill-down (sites filtrés par `pieve_slug` au clic polygone pieve)
- Toute correction `doyenne_contemporain_slug` sans `pieve_slug` correspondant **engendre une régression visuelle latente** : le site apparaît sur le bon doyenné en N1/N2 mais reste rattaché à une pieve d'un autre doyenné en N3
- Si la pieve cible n'existe pas (cas migration entre doyennés sans pieve directe correspondante) : arbitrage Soleil Phase A obligatoire (création pieve OU rattachement à pieve adjacente avec polygone)

**Application** :
- Toute PR fix rattachement modifie idéalement `pieve_slug` ET `doyenne_contemporain_slug` simultanément
- Vérification post-édition : pour chaque site touché, `pieve_slug` doit appartenir à une pieve dont `doyenne_contemporain_majoritaire == doyenne_contemporain_slug` (cohérence inter-niveaux)
- Si le `pieve_slug` cible est inconnu : remonter Phase A, ne pas modifier uniquement `doyenne_contemporain_slug`

**Applicable à :** tous les sprints futurs touchant `sites_patrimoine.json` (rattachements, audits visuels, refactorisation territoriale).

**Origine :** Lot 1 PR #585 (situation problème, doyenné seul corrigé), lots 4 PR #591 + 8 PR #599 + 9 PR #601 (pattern complétion observé puis formalisé), série fix N3 (14-15 mai 2026).

---

### BP-FUSION-POLYGONE-ARBITRAGE-001 — Fusion polygone admise sous arbitrage Soleil Phase A

**Contexte initial :** Avant la série fix N3 (mai 2026), la doctrine implicite Tellux était de ne pas modifier `docs/data/pieves_polygons.json` hors livraison Cowork canonique (Brief 17 livraison Casta v2 = 47 pieves). La série fix N3 a fait évoluer cette doctrine : certaines pieves "non-cliquables" sont apparues à l'audit visuel Soleil sur plusieurs doyennés (Golo NE Orezza, Cortenais 2 zones, PTV 3 zones, Plaine Orientale 3 zones anti-conflit, total ~10 fusions cumulées sur 4 PRs de la série).

**Doctrine acquise** : la fusion polygone est admise dans `pieves_polygons.json` **sous arbitrage Soleil explicite Phase A**. Décision motivée par contexte historique (deux pieves canoniques distinctes vs erreur de découpage moderne), choix slug cible minimisant les conflits homonymie inter-doyennés.

**Fusions appliquées série fix N3 (14-15 mai 2026)** :
- PR #591 lot 4 Golo : `pieve_ampugnani` (NE Orezza, 3 sites) → `pieve_orezza` (option pragmatique : suppression polygone, retag sites)
- PR #595 lot 6 Cortenais : 2 zones non-cliquables identifiées (Altiani/Sermano + Bustanico) → option simplifiée retag vers `pieve_bozio` + `pieve_giovellina` (pas de fusion polygone, différée)
- PR #601 lot 9 PTV : 3 zones non-cliquables → option simplifiée retag vers `pieve_ornano` + `pieve_istria` (pas de fusion polygone, différée)
- PR #605 lot 11 Plaine Orientale : **3 fusions polygones anti-conflit homonymie** (option complète) :
  - `pieve_bozio` (PO, 2 sites) → `pieve_alesani` (évite conflit pieve_bozio Cortenais)
  - `pieve_campoloro` (9 sites) → `pieve_moriani` + suppression polygone
  - `pieve_verde` (PO, 8 sites) → `pieve_fiumorbo` + suppression polygone (évite conflits pieve_verde résiduels)
- PR #607 lot 12 Cap compléments : pas de fusion (4 retags pieve_slug)

**Choix doctrinal slug cible (anti-conflit)** : quand deux pieves de doyennés différents partagent un slug, conserver celui qui minimise les conflits homonymie inter-doyennés (ex. lot 11 garde `pieve_alesani` plutôt que `pieve_bozio` car `pieve_bozio` existe déjà en Cortenais).

**Template plan technique fusion (Phase A)** :
1. Identification précise des polygones source + cible (vérifier existence dans `pieves_polygons.json`)
2. Vérification absence de chevauchement topologique critique (centroïdes, bbox)
3. Stratégie fusion :
   - **Option pragmatique** (suppression polygone source + retag sites vers cible existante) — utilisée PR #591, #595, #601 quand polygones disjoints ou fusion géométrique risquée
   - **Option complète** (suppression polygone source + retag + éventuellement extension géométrique cible) — utilisée PR #605 quand cible préexiste avec polygone valide
   - **Union géométrique** (concaténation coordonnées, MultiPolygon Leaflet) — réservée cas avancés sous arbitrage Soleil
4. Liste sites à migrer du polygone source vers polygone cible
5. Suppression entrée polygone source du JSON (ou conservation si chevauchement Cortenais/PO comme `pieve_bozio` PR #605)
6. Mise à jour `_meta` si applicable

**Application** :
- Ne pas modifier `pieves_polygons.json` sans arbitrage Soleil Phase A explicite
- Documenter dans le commit le slug source supprimé/conservé, slug cible étendu, N sites migrés, justification anti-conflit le cas échéant
- Préserver doctrine GPS publié strict sur sites migrés (champs `lat`/`lon` intacts)
- Valider en preview que les sites migrés apparaissent bien au clic du polygone cible

**Applicable à :** tous les sprints futurs touchant `pieves_polygons.json` (refactorisation territoriale, fusions anti-conflit, intégration livraison Cowork). Les modifications hors fusion (création/suppression de pieve canonique) restent hors scope BP — réservées aux livraisons Cowork validées Soleil.

**Origine :** PR #591 (situation problème, suppression polygone `pieve_ampugnani`), PRs #595 + #601 (option simplifiée), PR #605 (option complète anti-conflit homonymie). Série fix N3 (14-15 mai 2026).

---

## Dettes fermées récemment

- **CORPUS-DOUBLONS-FARINOLE-001** (21 mai 2026, branche `data/pip-corrections-vague-2`) — **résolue par merge**. Les entrées `ophi_farinole` et `mine_de_magnetite_de_farinole` désignaient le même site (nom, INSEE 2B109, commune, pieve, doyenné identiques) — doublon confirmé par l'investigation BRGM 80-RDM-003-FE (verdict A, `_drafts/AUDIT_DOUBLON_FARINOLE.md` + `_drafts/FARINOLE_DECISION.json`). `ophi_farinole` supprimé ; `mine_de_magnetite_de_farinole` conservé comme entrée primaire, enrichi de la `description` / `description_em` scientifiques d'`ophi_farinole`, reclassé « Site naturel remarquable » / `remarquables_geologiques`, GPS corrigé 42.73/9.332 → 42.7333/9.345 (`gps_source` BRGM 80-RDM-003-FE + Visorando GPX). Le gisement d'amiante d'Olmeta-du-Cap, distinct, a été ajouté séparément (`mine_amiante_olmeta_du_cap`).
- **PIEVE-MAPPING-AMONT-DESYNCHRO-001** (21 mai 2026, Étape 5 PR C, branche `feat/patrimoine-mapping-amont-cleanup-d3-d4`) — **résolue**. Dette HAUTE. La voie-a (rebuild via `build_pieves_polygons.py`) est redevenue viable : câblage du mapping v4 (D1) dans le pipeline, rebuild → `pieves_polygons.json` v7, 50 pieves, 0 zombie, 0 pieve manquante. Le mapping amont et le dérivé prod sont désormais alignés (`validate_mapping.py` REV2 vert + nouveau containment check renforcé mapping↔dérivé à 0 écart). 11 pieves changent de composition vs l'ancienne prod incohérente post-voie-b — refonte assumée, arbitrage Soleil 2026-05-21.
- **POLYGONE-INVALID-SELF-INTERSECTING-001** (21 mai 2026, Étape 5 PR C) — **résolue par le rebuild**. Les 3 polygones invalides (`pieve_fiumorbo`, `pieve_gulfo_d_aiacciu`, `pieve_vallerustie`) sont régénérés proprement par `unary_union` + `simplify(preserve_topology=True)` : 0 polygone invalide post-rebuild (vérifié shapely `is_valid`).
- **COMMUNES-COUNT-OBSOLETE-POST-VOIE-B-001** (21 mai 2026, Étape 5 PR C) — **résolue par le rebuild**. `communes_count`, `polygon` et `stats` sont recalculés de façon cohérente depuis le mapping v4. `stats.pieves_count` 51→50, `stats.total_communes` 347→360.
- **MAPPING-PRE-STRATD-AD-HOC-TRANSFERS-UNDOCUMENTED-001** (21 mai 2026, Étape 5 PR C) — **résolue**. Les ~26 communes ex-zombies sans source (ampugnani/campoloro/giovellina résiduel) assignées best-effort PIP en v4 sont désormais entérinées : le rebuild v4 est accepté comme nouvelle référence cartographique (arbitrage Soleil 2026-05-21). L'origine historique reste non tracée mais l'état v4 fait foi.

- **PATRIMOINE-B-ZONES-TIER2-FAVORABLE-001** (20 mai 2026, branche `feat/b-zones-tier2-8-sites`) — résolue par la PR B B-ZONES. Les 8 sites Tier 2 favorables ont reçu `is_zone` + `zone_geometry` : 5 monts (`monte_cinto`, `monte_san_petrone`, `monte_stello`, `monte_d_oro`, `monte_renoso`) en octogones manuels rayon ~2-3 km autour du sommet, `capu_rossu` (OSM relation 9376817 natural=cape), `lac_de_nino` (OSM way 28890021 + buffer ~600 m pour englober les pozzines), `cap_corse_extreme_nord` / Capu Bianchi (pointe manuelle). Compteur B-ZONES 15 → 23. Smoke test PIP Corse 23/23. `cap_corse_extreme_nord` reste orphan de doyenné (cf. `PATRIMOINE-ORPHANS-INVISIBLES-001`, non bloquant).
- **CASTEDDU-BASTELICA-COORD-DOUTE-001** (18 mai 2026, branche `chore/patrimoine-sprint-hygiene-pre-feder`) — résolue par alignement métadonnées sur géo réelle (option a Soleil). PIP commune sur coord verrouillé (42.217, 9.255, Brief 39quater) = `Altiani 2B012` / `pieve_altiani` / `doyenne_cortenais`. Corrigé : `commune_insee 2A031 → 2B012`, `commune_nom Bastelica → Altiani`, `pieve_rogna → pieve_altiani`. Nom site « Casteddu de Bastelica » conservé (toponymie homonyme à clarifier post-FEDER, non bloquant).
- **TOUR-ISOLELLA-INSEE-DISCORDANCE-001** (18 mai 2026, branche `chore/patrimoine-sprint-hygiene-pre-feder`) — résolue par correction INSEE seule. `commune_insee 2A258 (Renno faux) → 2A228 (Pietrosella officiel)`, code croisé via `scripts/.cache/communes-2A.geojson`. `pieve_ornano` + `doyenne_prunelli_taravo_valinco` déjà cohérents (mapping v1 Pietrosella → pieve_ornano), pas de retag nécessaire.
- **PIEVE-PIEDIPARTINO-INSEE-DISCORDANCE-001** (18 mai 2026, branche `chore/patrimoine-sprint-hygiene-pre-feder`) — résolue par correction INSEE + retag complet (doctrine `BP-FIX-RATTACHEMENT-COMPLET-001`). `commune_insee 2B231 (Pigna faux) → 2B221 (Piedipartino officiel)`, `pieve_aregno → pieve_orezza` (mapping v1 Piedipartino), `doyenne_balagne → doyenne_du_golo` (pieve_orezza majoritaire). Coord 42.31/9.36 inchangé (precision ±2km centroid_a_preciser pré-existant, hors-scope hygiène).
- **FARINOLE-COORD-DOUTE-001** (18 mai 2026, audit Phase A sprint hygiène) — **fermée par invalidation** (faux positif). Vérification prod : `tour_de_farinole` lat 42.7336/lon 9.3626 ≈ centroïde commune Farinole 42.73/9.34, cohérent. `pieve_patrimonio` / `doyenne_du_cap` cohérents. Anomalie initiale liée à un état antérieur du polygone pieve_nebbiu (résolu par Stratégie D + audit visuel Soleil 14 mai). Distincte de `SITES-PATRIMOINE-TOURS-FARINOLE-GPS-PRECIS-001` qui concerne la précision GPS (reste ouverte).
- **PIEVE-VENACO-OVERFLOW-VISUEL-GEOMETRIQUE-001** (18 mai 2026, audit Phase A sprint hygiène) — fermée comme **cosmétique acceptable Phase 1 beta**. Vérification : `pieve_venaco` 9 communes, doy_maj cortenais 99.76% (0 mismatch commune). Débordement visuel résiduel = simplification polygone tolerance 0.0005°. Refactor pipeline simplification post-FEDER si besoin (clipping aux frontières doyennés).
- **PIEVE-SORROINSU-CINARCA-POST-MIGRATION-PHASE2-001** (18 mai 2026, audit Phase A sprint hygiène) — **fermée par invalidation**. Vérification : `pieve_sorroinsu` 4 communes (Guagno/Orto/Poggiolo/Soccia) + `pieve_cinarca` 9 communes, polygones cohérents post-Phase 2. Aucun trou visuel détecté en preview. Équilibre OK.

- **EDIFICES-ROMANS-RENAME-001** (15 mai 2026, branche `refactor/patrimoine-religieux-rename`) — renommage de l'identifiant d'axe `edifices_romans` en `patrimoine_religieux` dans toute la codebase front (data + scripts). Motivé par l'élargissement réel du scope : la couche contient des édifices romans tardifs (XVᵉ), des couvents, des cathédrales, et des édifices non strictement romans (Cargèse grec/latin). Champ `categorie` aussi mis à jour de `'Édifice roman'` à `'Patrimoine religieux'`. Périmètre : `docs/data/sites_patrimoine.json` (314 occurrences), `docs/data/sites_corse.json` (316 occ.), `scripts/consolidate_sites.py` (9 occ., déprécié), `scripts/generate_audit_md.py` (2 occ., legacy), `DETTES_TECHNIQUES.md` (2 mentions incidentes). Zéro impact runtime : `patrimoine.html` consomme `axe_corpus` génériquement (l.470), aucune valeur hardcodée dans le HTML/JS. Aucun changement Supabase (champ front uniquement).

- **PATRIMOINE-HASH-DEEPLINK-CADRAGE-001** (14 mai 2026, PR #577 commit `4dbee05`) — résolue lors de l'implémentation niveau 3 patrimoine. Le fix a introduit un flag `opts.animate=false` dans `enterNiveau2View` et `enterNiveau3View` : quand le pipeline `applyHash → enterNiveau2View → enterNiveau3View` est déclenché par hash deeplink, on remplace `flyToBounds` (animé) par `fitBounds` (synchrone), ce qui assure le cadrage sur les markers DOM créés mais non encore cadrés. Validation preview locale 5 scénarios (deeplink #balagne, deeplink #extreme_sud/sartene, navigation N1→N2→N3, retour bouton pieve, retour bouton corse) avant merge. Cause racine du bug initial : `flyToBounds` était joué avant l'attache des markers à la couche, viewport restait centré ailleurs. Bonus : commit `4dbee05` ajoute aussi le cancel du timer fade N1→N2 pendant le chain deeplink synchrone (sinon les markers du doyenné se ré-attachaient 250ms après le detach N3).
