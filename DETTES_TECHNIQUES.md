...
### FARINOLE-COORD-DOUTE-001 — `tour_de_farinole` coord à frontière pieve (point en mer probable)

**Description :** Site `tour_de_farinole` a coord (lat 42.7333, lon 9.3333) = centroïde commune Farinole (cohérent avec commune ouest Cap), mais le point tombe à ~700 m de la frontière pieve_nebbiu / pieve_patrimonio sans être contenu dans aucun polygone doyenné (`shapely.contains` retourne NONE pour les 9 doyennés). Probable point en mer dans le golfe ouest du Cap. Tag actuel `pieve_nebbiu` / `doyenne_du_cap` maintenu en l'état (cas suspendu option D arbitrage Soleil 2026-05-17). Distinct de `SITES-PATRIMOINE-TOURS-FARINOLE-GPS-PRECIS-001` qui concerne la précision GPS (centroïde commune vs GPS précis Mérimée) — ici le problème est que même le centroïde ne tombe dans aucun polygone exploitable.

**Priorité :** Faible

**Condition de déblocage :** Recherche GPS précis tour de Farinole (Mérimée POP PA00125391, Wikipedia FR si fiche dédiée, ou Wikidata). Si GPS précis trouvé compatible polygone pieve_nebbiu → mise à jour coord + retag selon géo. Si GPS confirme position côtière en mer (tour génoise littorale isolée) → flag explicite `site_marin: true` + tag ad hoc.

**Identifiée :** 17 mai 2026 (audit Phase A cat 2 cross-doyennés Cowork, Soleil arbitrage option D suspends).

---

### CASTEDDU-BASTELICA-COORD-DOUTE-001 — `casteddu_bastelica` doute homonymie/coord

**Description :** Site `casteddu_bastelica` est tagué `pieve_rogna` / `doyenne_cortenais` (current). Coord tombe géométriquement dans pieve_rogna polygone ET doyenne_cortenais polygone. MAIS le nom "Bastelica" suggère la commune Bastelica (sud d'Ajaccio, doyenné Prunelli-Taravo-Valinco), pas une localité Cortenais. Deux hypothèses : (a) coord erronée — il y a un autre site "Casteddu" à Bastelica qui devrait être au sud, (b) nom trompeur — un casteddu réel en Cortenais qui partage par hasard "bastelica" dans son slug (autre Bastelica, hameau ?). Tag inchangé (cas suspendu option D arbitrage Soleil 2026-05-17).

**Priorité :** Faible

**Condition de déblocage :** Audit toponymique : vérifier dans Mérimée + Wikipedia si "Casteddu Bastelica" est un site Cortenais ou si la coord doit être recadrée sur Bastelica/PTV. Si recadrage : retag (pieve PTV, doyenne_prunelli_taravo_valinco). Si nom Cortenais légitime : ajouter note de désambiguïsation dans description.
...
### PIEVE-VENACO-OVERFLOW-VISUEL-GEOMETRIQUE-001 — Débordement visuel pieve_venaco vers PO

**Description :** Débordement visuel du polygone `pieve_venaco` vers Plaine Orientale signalé par Soleil mais **0 mismatch commune** (audit Cowork 2026-05-17). Cause = simplification polygone (`tolerance=0.0005°`) ou `multipolygon_dropped_areas`. Pas urgent.

**Priorité :** Faible.

**Condition de déblocage :** Post-Stratégie D (Phase D ≥5) : étape de containment polygonale stricte (clipping aux frontières doyennés).

**Identifiée :** 17 mai 2026 (audit Cowork Phase A).
...
### TOUR-ISOLELLA-INSEE-DISCORDANCE-001 — `tour_d_isolella_sette_navi` INSEE Renno mais géo Ajaccio sud

**Description :** Site `tour_d_isolella_sette_navi` a `commune_insee=2A258` (Renno) mais ses coord lat/lon correspondent à la côte sud d'Ajaccio (zone Sette Navi/Isolella). Anomalie INSEE/géo : Renno est dans l'arrière-pays Vico, pas sur la côte ajaccienne. Le site est tagué `pieve_ornano` (cohérent avec sa géo) mais sa commune INSEE est erronée.

**Priorité :** Faible (impact UX nul, juste champ commune_insee incohérent en interne).

**Condition de déblocage :** Post-FEDER : vérifier coord lat/lon vs commune réelle (probable Pietrosella 2A218 ou Coti-Chiavari 2A097). Corriger commune_insee. Pas de retag pieve (géo Ajaccio correcte).

**Identifiée :** 18 mai 2026 (Stratégie D Phase 2, arbitrage Soleil Q-3 conservation pieve_ornano).

---

### PIEVE-SORROINSU-CINARCA-POST-MIGRATION-PHASE2-001 — Vérif équilibre pieves rétrécies post-Phase 2

**Description :** Suite Stratégie D Phase 2, `pieve_sorroinsu` réduite de 7 → 4 communes (Guagno, Orto, Poggiolo, Soccia) et `pieve_cinarca` de 9 → 9 (Coggia migré vers sagone, mais base mapping v1 = 10 donc net stable 9). Polygones recalculés via shapely union. À vérifier en preview que les polygones rétrécis restent visuellement cohérents et n'introduisent pas de trous dans le doyenné PVS.

**Priorité :** Faible.

**Condition de déblocage :** Validation visuelle preview + prod. Si trous visuels détectés, étendre les pieves voisines (vico, ornano) pour combler.

**Identifiée :** 18 mai 2026 (Stratégie D Phase 2).

---

### PIEVE-PIEDIPARTINO-INSEE-DISCORDANCE-001 — `pont_genois_de_piedipartino` INSEE Pigna ≠ nom

**Description :** Site `pont_genois_de_piedipartino` a `commune_insee=2B231` qui est Pigna (centre-Balagne, dans pieve_aregno post-split). Le nom du site suggère Piedipartino (commune Cortenais ou Castagniccia). Anomalie INSEE/nom. Le retag Phase 2 applique pieve_aregno selon INSEE déclaré (cohérent avec doctrine BP-FIX-RATTACHEMENT-COMPLET-001), mais géo possiblement incohérente avec le nom.

**Priorité :** Faible.

**Condition de déblocage :** Post-FEDER : audit toponymique. Si vrai Piedipartino → corriger INSEE + retag pieve cible (probable pieve_orezza). Si nom trompeur (site réellement à Pigna) → laisser tel quel + ajouter note de désambiguïsation.

**Identifiée :** 18 mai 2026 (détectée par retag Phase 2).
