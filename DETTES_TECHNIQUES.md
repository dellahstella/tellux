...
**Description :** La constante `ILLUSTRATED_SPOTS` (Set hardcodé dans `patrimoine.html`) liste les slugs dont les markers doivent apparaître en N1 macro-vue. Toute évolution de corpus (ajout/retrait de visuels) doit être manuellement synchronisée avec ce Set, sinon dérive entre HTML et data. Brief Phase B 2026-05-11 a retiré 2 slugs invalides (`cirque_de_bonifato`, `massif_du_haut_asco` initial) — whitelist 14 → 12. Sprint curation N1 (2026-05-11) a élargi à 24 slugs via Item 6 (incluant `massif_du_haut_asco` une fois son visuel `_v1_medium.webp` confirmé + 11 autres slugs). Drift sous-jacent : `sites_patrimoine.json` contient ~96 sites avec `illustre: true` mais seuls 24 sont en N1 (curation éditoriale manuelle). Pas d'auto-detection de la dérive.

**Priorité :** Basse.

**Condition de déblocage :** (a) Soit dériver `ILLUSTRATED_SPOTS` à la volée depuis `sites_patrimoine.json` (champ `illustre: true` + audit `priorite`), (b) soit ajouter un script de check CI qui valide la cohérence entre le Set et la liste effective des visuels. Identifiée Phase A Cowork (2026-05-11) symptôme S5. **Statut sprint curation N1 : whitelist 12→24, drift 96→24 toujours ouvert mais sous contrôle éditorial.**
...
**Risque :** toute modification data via le pipeline canonique ne se propage pas au runtime. Les corrections sont éphémères tant que la chaîne n'est pas réparée.

**Action requise (sprint séparé) :**
- Réparer `consolidate_sites.py` pour écrire au bon chemin
- Auditer les 22 sites perdus (qui sont-ils, doivent-ils être réintégrés ?)
- Décider d'une seule source de vérité (canonical vs publié)

**Priorité :** moyenne. Pas bloquant Phase 1 mais à traiter avant prochaine vague de modifications data significatives.
