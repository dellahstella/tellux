...
**Description :** Sur les 124 spots du manifest patrimoine, environ 105 sont actuellement invisibles dans la navigation patrimoine.html v1 (post-Brief 8 révisé du 4 mai 2026 — pivot sur 11 doyennés contemporains, vision macro Soleil). Critères : seuls les 14 spots illustrés (liste blanche `ILLUSTRATED_SPOTS` validée Brief 8) sont affichés au niveau 1 ; les 5 spots-diocèse (slug commençant par `diocese_`) sont exclus ; les ~105 autres sont masqués (ni illustrés, ni rattachés à un calque visible). Liste exacte loggée au boot via `console.log('[Tellux patrimoine] Spots non illustrés masqués: …')` (DevTools, pas user-facing). Cohérent avec la doctrine « tri/rangement = phase ultérieure ». Note : la formulation antérieure (drill-down 2 niveaux Brief 8 initial, ~43 orphans invisibles) est obsolète depuis le pivot vers la vision macro 1 niveau du Brief 8 révisé.

**Priorité :** Moyenne (différée)

**Condition de déblocage :** Phase tri/rangement ultérieure. Trois leviers possibles :
1. Rattachement manuel à un doyenné via overrides JSON dans le pipeline (ajout d'un champ `doyenne_override` dans le manifest spots), permettant l'affichage agrégé d'un décompte au hover du polygone doyenné.
