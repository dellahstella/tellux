...
### POLYGONE-INVALID-SELF-INTERSECTING-001 — 3 polygones prod géométriquement invalides

**Statut :** Ouverte, priorité moyenne (PR C).

**Date :** 2026-05-20

**Description :** 3 polygones de `docs/data/pieves_polygons.json` sont géométriquement invalides (self-intersecting selon shapely `is_valid`) : `pieve_fiumorbo`, `pieve_gulfo_d_aiacciu`, `pieve_vallerustie`. Conséquence d'unions/simplifications « voie-b » successives. Le PIP D1 les répare à la volée via `buffer(0)` ; le dérivé prod reste invalide.

**Condition de déblocage :** Rebuild propre PR C (voie-a `build_pieves_polygons.py`) qui régénère des polygones valides depuis le mapping v4.

---

### COMMUNES-COUNT-OBSOLETE-POST-VOIE-B-001 — communes_count prod obsolète pour 16/50 pieves

**Statut :** Ouverte, priorité moyenne (PR C).

**Date :** 2026-05-20

**Description :** Pour 16 des 50 pieves de `pieves_polygons.json`, le champ `communes_count` ne correspond plus à la géométrie du `polygon` (écarts jusqu'à ±15, ex. `pieve_orezza` count=14 vs PIP=29). Cause : patches « voie-b » qui ont modifié `polygon` ou `communes_count` indépendamment. `stats.pieves_count` (51) diverge aussi de `len(pieves)` (50), et `stats.total_communes` (347) de 360. `validate_mapping.py` REV2 ne compare donc PAS au `communes_count` prod.

**Condition de déblocage :** Rebuild PR C — le pipeline recalcule `communes_count`, `polygon` et `stats` de manière cohérente depuis le mapping v4.

---

### MAPPING-PRE-STRATD-AD-HOC-TRANSFERS-UNDOCUMENTED-001 — transferts pré-Strat-D non tracés

**Statut :** Ouverte, basse priorité.

**Date :** 2026-05-20

**Description :** Les absorptions historiques des zombies `pieve_ampugnani` (14 communes → orezza), `pieve_campoloro` (8 → moriani) et le résiduel `pieve_giovellina` (4 → talcini/bozio/orezza) ont été faites par des scripts ad hoc pré-Strat-D absents du repo (`phase_qw_pieves.py` ne contient aucune réaffectation). En D1, ces 26 communes ont été réassignées en v4 par PIP polygones prod best-effort (arbitrage Soleil 2026-05-20), documentées dans les `transferts` v4 avec la note dédiée.

**Condition de déblocage :** Si les scripts d'absorption sont retrouvés, comparer leurs cibles aux transferts v4. Sinon, dette fermable après validation du rebuild PR C.
...
**Amendement 2026-05-18 (Stratégie D Phase 1) — ESCALADE PRIORITÉ : Haute** : la voie (a) régénération propre via `build_pieves_polygons.py` produit aujourd'hui des **régressions inacceptables** : (1) 4 pieves zombies réapparaissent (`pieve_giovellina`, `pieve_caccia`, `pieve_ampugnani`, `pieve_campoloro`) car leurs communes sont encore dans mapping v1 alors qu'elles ont été fusionnées via scripts ad hoc dans le passé sans synchroniser le mapping ; (2) `pieve_zicavo` disparaît du build car absente du mapping v1+v2 (présente en prod via patch direct dérivé) ; (3) `pieve_patrimonio` produit 6 communes au build vs 9 en prod (créée via `scripts/create_pieve_patrimonio.py` sans mise à jour mapping). **Conséquence Stratégie D** : voie (b) patch direct du dérivé adoptée (cf. `scripts/phase_strat_d_patch_derive.py`), avec compteurs légèrement décalés vs prod pour les pieves modifiées (`pieve_nebbiu` 17→9, `pieve_patrimonio` 9→6) — visuels acceptables Phase 1 mais perfectible. **PR cleanup mapping amont OBLIGATOIRE post-FEDER** (rappel explicite Soleil 2026-05-17) : (a) résoudre les 4 pieves zombies (transferts v3 vers leurs cibles d'absorption documentées), (b) sourcer `pieve_zicavo` (communes_insee), (c) compléter `pieve_patrimonio` (3 communes Cap manquantes), (d) re-tester régénération propre via script + `--strict-containment`.

**Amendement 2026-05-20 (Étape 5 PR A — D1 cleanup mapping amont, brief REV2) — PROGRESSION** : la PR A D1 a aligné le mapping amont sur la prod (50 pieves). Le mapping mergé v1+v2+v3+**v4** est désormais cohérent : 0 zombie, 0 pieve manquante, 360 communes mappées (`scripts/validate_mapping.py` REV2 vert). Méthode REV2 : la prémisse REV1 « PIP polygones prod = source de vérité unique » a été **abandonnée** après blocage Phase A — PIP ne reproduit pas `communes_count` prod pour 16/50 pieves (polygones obsolètes voie-b) et 3 polygones sont invalides. Source de vérité retenue : scripts ad hoc `phase_strat_d_phase2_splits.py` + `phase_strat_d_patch_derive.py` (listes explicites) ; PIP best-effort pour les ~30 communes sans source (zombies pré-Strat-D + `pieve_zicavo`), arbitrage Soleil 2026-05-20, dettes `MAPPING-PRE-STRATD-AD-HOC-TRANSFERS-UNDOCUMENTED-001` + `MAPPING-PREQW-ORIGIN-UNKNOWN-001`. **Reste avant clôture** : D2 (archivage scripts `phase_*.py`, PR B) puis D3/D4 (rebuild voie-a propre via `build_pieves_polygons.py` depuis v4, PR C). Cette dette **reste ouverte** — clôture en PR C après rebuild validé.
