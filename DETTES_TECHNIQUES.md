...
**Statut : EN VOIE D'ÉCLAIRCISSEMENT 2026-05-05 (Brief 17 Phase A)** — mécanisme d'override actif via `_drafts/SPOT_DOYENNE_OVERRIDES.json`. Soleil peut désormais rattacher manuellement chaque orphan à un doyenné (et optionnellement à une pieve) au fil des sessions, sans modifier le pipeline ni le mapping Cowork. Le runtime `patrimoine.html` charge le fichier au boot et applique les overrides : les markers correspondants apparaissent au drill-down niveau 2 du doyenné cible. Logging au boot : `[Tellux patrimoine] Overrides spots orphans : X adressés | Y ignorés | Z encore invisibles`. Le squelette est livré vide (`overrides: {}`) et se remplit progressivement.

**Description :** Sur les 124 spots du manifest patrimoine, environ 105 sont actuellement invisibles dans la navigation patrimoine.html v1 (post-Brief 8 révisé du 4 mai 2026 — pivot sur 10 doyennés contemporains, vision macro Soleil). Critères : seuls les 14 spots illustrés (liste blanche `ILLUSTRATED_SPOTS` validée Brief 8) sont affichés au niveau 1 ; les 5 spots-diocèse (slug commençant par `diocese_`) sont exclus ; les ~105 autres sont masqués (ni illustrés, ni rattachés à un calque visible). Liste exacte loggée au boot via `console.log('[Tellux patrimoine] Spots non illustrés masqués: …')` (DevTools, pas user-facing). Note : la formulation antérieure (drill-down 2 niveaux Brief 8 initial, ~43 orphans invisibles) est obsolète depuis le pivot vers la vision macro 1 niveau du Brief 8 révisé.
...
**Description :** Le mapping pieves Cowork livré le 4 mai 2026 et utilisé par le pipeline `scripts/build_pieves_polygons.py` (Brief 9 drill-down niveau 2) couvre 39 pieves alignées sur le manifest `PATRIMOINE_POLYGONS.pieves` du Brief 2, et non les 47 pieves Casta canoniques. Décision actée Soleil 2026-05-04 lors du retour Cowork. Cinq pieves Casta sont implicitement absorbées dans des voisines : `Casacconi → Mariana`, `Tavagna → Moriani`, `Filosorma → Vico` (couverte par Vico), `Mezzana → Ajaccio` (couverte par Ajaccio), `Cap-Corse-Luri → Cap` (couverte par Brando/Canari/Pino/Rogliano/Nonza). Trois autres pieves Casta supplémentaires sans rattachement clair côté Cowork (à investiguer si cohérent avec la canonicité Casta stricte). Acceptable visuellement car les communes médiévales de ces 5 pieves absorbées sont redistribuées dans les voisines (couverture exhaustive des 360 communes corses préservée). La mention « 47 pieves Casta canoniques » dans la formulation initiale du Brief 9 (section 2 contexte) est donc factuellement plus large que ce qui est rendu côté UI v1.

**Priorité :** Faible (différée, non bloquante — la couverture territoriale reste exhaustive et la dichotomie médiéval/contemporain est déjà assumée comme stratification narrative)

**Condition de déblocage :** Session ultérieure de relecture canonique Casta avec arbitrage Soleil sur les 8 pieves manquantes (5 absorbées + 3 sans rattachement clair). Si restauration souhaitée : enrichir le mapping Cowork (`_drafts/pieves_communes_mapping.json`) ou créer overrides explicites dans `_drafts/PIEVE_OVERRIDES.json`, puis re-run `python scripts/build_pieves_polygons.py`. Code applicatif `patrimoine.html` ne nécessite aucune modification (substitution data uniquement).
