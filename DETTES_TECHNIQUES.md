...
### SITES-PATRIMOINE-JSON-L13034-001 — Virgule trailing ligne 13034 dans `sites_patrimoine.json`

**Description :** `sites_patrimoine.json` ligne 13034 contiendrait une virgule trailing avant `}` selon le smoke test `python3 scripts/sync_cross_app.py --dry-run` signalé par Cowork pendant M3 (13 mai 2026). Extrait suspect : `"visuel": "docs/assets/visuels/plateau_du_coscione_tellux_v2",`. Si confirmée, cette virgule trailing bloque le parsing JSON et donc le dry-run de `sync_cross_app.py`. Hors périmètre M3 (verrou : ne pas toucher `sites_patrimoine.json` canonical pendant M3).

**Priorité :** Moyenne (bloque `sync_cross_app.py --dry-run`, smoke test post-M3)

**Condition de déblocage :** Audit ciblé de la ligne 13034 de `sites_patrimoine.json` (et `python -m json.tool sites_patrimoine.json` pour valider l'ensemble) + correction syntaxique si confirmée. ~15 min Cowork ou Code.

**Identifiée :** 13 mai 2026 (Cowork pendant M3, signalée hors périmètre de la branche).
