...
**Description :** Le mapping `RADON_CLASS_BY_LITHOLOGY` (correspondance lithologie → classe radon 1/2/3) est défini en deux endroits dans `app.html` : dans `calcSubstrateContext` (ligne ~3939) et dans `calcRadonPotential` (ligne ~4102). Risque de dérive entre les deux copies si l'une est mise à jour sans l'autre. Le contenu actuel est identique et stable. Identifiée le 26 avril 2026 lors de la cartographie d'extraction du moteur (`docs/em-mairie/tellux-engine-extraction-plan.md` section 6.7).

**Priorité :** Faible (cosmétique, contenu actuel identique)

**Condition de déblocage :** Centraliser le mapping en un seul export (`data/radon-classification.js` ou équivalent) lors de l'extraction du moteur. Référence : `docs/em-mairie/tellux-engine-extraction-plan.md` section 6.7.

---

### HELPERS-INLINE-CONSTS — Constantes physiques inline dans le moteur de calcul

**Description :** Plusieurs constantes physiques sont définies inline dans le corps de fonctions du moteur de calcul de `app.html`, recréées à chaque appel et non testables en isolation. Liste identifiée le 26 avril 2026 lors de la cartographie du moteur (`docs/em-mairie/tellux-engine-extraction-plan.md` section 6.8) :
...
**Mise à jour 2026-05-21 (audit cohérence sources) :** L'audit `_drafts/AUDIT_COHERENCE_SOURCES_PATRIMOINE.md` confirme l'architecture (option A entérinée : `sites_patrimoine.json` source de vérité runtime, `sites_corse.json` déprécié sans consommateur runtime). L'archivage de `sites_corse.json` a été préparé puis **suspendu** : 2 scripts de maintenance le référencent encore — `scripts/brief_pipeline.py` (l.169, l'itère pour appliquer les corrections de briefs ; skippe proprement si le fichier est absent grâce au garde `if path:`) et `scripts/corpus_health_check.py` (l.160, le charge pour la validation JSON ; signalerait une erreur « fichier absent » si déplacé). Aucun consommateur runtime ni CI. Le nettoyage définitif (archivage de `sites_corse.json` + retrait de la référence dans ces 2 scripts) reste à faire dans un chantier dédié.
