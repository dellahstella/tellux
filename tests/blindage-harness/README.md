...
L'extraction du moteur vers `lib/tellux-engine.js` est conçue (suivi interne) mais conditionnée à plusieurs prérequis non encore réunis (retour de relecture méthodologique externe, volume contributions niveau 1, disponibilité de pilotage). Ce harness est l'**enabler intermédiaire** : il rend la testabilité automatisée et la sensibilité possibles **sans attendre P0**, sans toucher au moteur, et reste compatible avec une migration ultérieure vers un runner de tests natif (`node:test`) une fois le moteur extrait.
