...
# Creation pieve_tavignano (2026-05-23) — mapping v6 :
# nouvelle pieve bassin moyen-bas Tavignano (9 transferts depuis 5 pieves
...
# Audit pieve_rogna degeneree (2026-05-23) — mapping v8 :
# resorption MultiPolygon degenere de pieve_rogna post-Tavignano.
...
# Fix zone orpheline Vico (2026-05-23) — mapping v10 :
# transfert Coggia (2A090) pieve_sagone -> pieve_vico pour resorber le gap
...
def main():
    ...
    # Creation pieve_tavignano (2026-05-23) — mapping v6 incremental.
    # 9 transferts + 1 pieves_added. Disparition pieve_ghisoni (1 commune unique
    # transferee, la pieve sera absente du JSON output post-build).
    mapping_v6 = None
    if MAPPING_V6_PATH.exists():
        with MAPPING_V6_PATH.open(encoding="utf-8") as f:
            mapping_v6 = json.load(f)
        print(f"[build] mapping v6 (tavignano) charge : "
              f"{len(mapping_v6.get('pieves_added', []))} pieves ajoutees, "
    ...
    # Audit pieve_rogna degeneree (2026-05-23) — mapping v8 incremental.
    # 2 transferts (Bustanico 2B045 -> pieve_bozio, Sermano 2B275 -> pieve_talcini).
    # Resorbe le MultiPolygon degenere de pieve_rogna post-Tavignano.
    mapping_v8 = None
    if MAPPING_V8_PATH.exists():
        with MAPPING_V8_PATH.open(encoding="utf-8") as f:
            mapping_v8 = json.load(f)
        print(f"[build] mapping v8 (rogna audit bustanico/sermano) charge : "
              f"{len(mapping_v8.get('pieves_added', []))} pieves ajoutees, "
    ...
    # Fix zone orpheline Vico (2026-05-23) — mapping v10 incremental.
    # 1 transfert Coggia (2A090) pieve_sagone -> pieve_vico.
    mapping_v10 = None
    if MAPPING_V10_PATH.exists():
        with MAPPING_V10_PATH.open(encoding="utf-8") as f:
            mapping_v10 = json.load(f)
        print(f"[build] mapping v10 (vico fix zone orpheline) charge : "
              f"{len(mapping_v10.get('pieves_added', []))} pieves ajoutees, "
    ...
    # Creation pieve_tavignano (2026-05-23) — appliquer v6 (meme pattern).
    # Cas particulier : disparition pieve_ghisoni (1 commune unique transferee
    # vers pieve_tavignano). La pieve sera absente de pieve_to_communes apres
    # transferts, donc absente du JSON output. Documente via removed_zombies
    # dans le patch v6.
    if mapping_v6:
        for p in mapping_v6.get("pieves_added", []):
            for insee in p["communes_insee"]:
                commune_to_pieve[insee] = p["slug"]
        for t in mapping_v6.get("transferts", []):
            insee = t["commune_insee"]
            to_p = t.get("vers_pieve")
            current = commune_to_pieve.get(insee)
            commune_to_pieve[insee] = to_p
            transferts_appliques.append({"insee": insee, "from": current, "to": to_p, "via": "v6"})
        for r in mapping_v6.get("renames", []):
            old_slug = r["from"]
            new_slug = r["to"]
            for insee, slug in list(commune_to_pieve.items()):
                if slug == old_slug:
    ...
    # Audit pieve_rogna degeneree (2026-05-23) — appliquer v8.
    if mapping_v8:
        for p in mapping_v8.get("pieves_added", []):
            for insee in p["communes_insee"]:
                commune_to_pieve[insee] = p["slug"]
        for t in mapping_v8.get("transferts", []):
            insee = t["commune_insee"]
            to_p = t.get("vers_pieve")
            current = commune_to_pieve.get(insee)
            commune_to_pieve[insee] = to_p
            transferts_appliques.append({"insee": insee, "from": current, "to": to_p, "via": "v8"})
        for r in mapping_v8.get("renames", []):
            old_slug = r["from"]
            new_slug = r["to"]
            for insee, slug in list(commune_to_pieve.items()):
                if slug == old_slug:
    ...
    # Fix zone orpheline Vico (2026-05-23) — appliquer v10.
    if mapping_v10:
        for p in mapping_v10.get("pieves_added", []):
            for insee in p["communes_insee"]:
                commune_to_pieve[insee] = p["slug"]
        for t in mapping_v10.get("transferts", []):
            insee = t["commune_insee"]
            to_p = t.get("vers_pieve")
            current = commune_to_pieve.get(insee)
            commune_to_pieve[insee] = to_p
            transferts_appliques.append({"insee": insee, "from": current, "to": to_p, "via": "v10"})
        for r in mapping_v10.get("renames", []):
            old_slug = r["from"]
            new_slug = r["to"]
            for insee, slug in list(commune_to_pieve.items()):
                if slug == old_slug:
