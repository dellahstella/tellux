...
**Ce que ce retrait apporte réellement n'est pas une hausse ni une baisse** : c'est que la valeur affichée cesse de dépendre de l'heure et de la saison de consultation. Le facteur passait au-dessus de 1 une bonne partie de l'année — sur ces périodes le retrait fait **baisser** les valeurs, pas monter. Les deux sens sont donnés ci-dessus pour cette raison.

**Deux régimes, et c'est le régime établi qui est annoncé ci-dessus.** Le terme BT n'entre dans le calcul qu'une fois sa grille téléchargée et construite (`btTermeInclus()` = `USE_BT_SEGMENTS && BT_SEGMENT_GRID`) : les valeurs ci-dessus sont mesurées **après** cette construction. Avant elle — c'est-à-dire à l'état par défaut au chargement de chaque visite, mesuré à 119 s en production le 2026-09-09 — le terme BT est absent du total, la part HTA est donc plus grande et les écarts sont plus amples, jusqu'à **+144 % / −26 %** aux mêmes points. Ce n'est ni un réglage ni une couche à activer : c'est un état transitoire que traverse chaque session.
...
- **`app.html`** — le badge « Réseau » (`#badge-reseau`, résumé conditions live) et le panneau détaillé associé (`#cond-sec-reseau`, `#res-charge`, `#res-charge-s`, sparkline 24 h) sont **retirés** : un badge qui continuait d'afficher une charge sans qu'aucun calcul ne la consomme aurait gardé l'apparence d'un intrant du modèle alors qu'il n'en est plus un. **Conséquence de portée à noter** : la ligne de résumé « Réseau électrique » du panneau Conditions (`#cond-summary-reseau`, qui affichait « Heure pleine / creuse / Charge normale ×N ») disparaît avec lui — elle dérivait du même facteur, elle n'a plus d'objet.
...
### Changed
- **`app.html`** (`IGRF14_GRID`) — remplace un plan linéaire (écart max 10 nT à ses propres 54 points, malgré un commentaire affirmant une évaluation IGRF-14 analytique) par une vraie évaluation analytique (coefficients IGRF-14 officiels NOAA/IAGA/BGS + variation séculaire degré 8), validée à **0,6 nT** de la référence BGS à Ajaccio (65 nT avant). **Effet visible pour qui compare** : les valeurs de champ magnétique statique affichées se déplacent de **70 à 200 nT selon la localisation** par rapport à l'ancienne grille — écart attendu, l'ancienne grille sous-estimait la courbure réelle du champ, pas une régression. Le ±100 nT affiché n'est pas retouché (portait sur l'ancien défaut d'interpolation spatiale, pas sur la dérive temporelle — à rediscuter séparément). PR [#1356](https://github.com/dellahstella/tellux/pull/1356).

### Notes
- **Dette ouverte** : une grille figée régénérée reste une grille figée — elle recommence à dériver dès le lendemain de sa régénération (~41 nT/an, variation séculaire IGRF-14). Remplacer une grille périmée par une grille fraîche supprime le symptôme du jour, pas la classe de défaut. Trois sorties posées, aucune tranchée à ce commit : évaluation à l'exécution depuis les coefficients IGRF-14 (sortie propre — le code de variation séculaire existe déjà et est validé à 0,6 nT) ; régénération au build avec date d'évaluation injectée (coût moyen, même effet) ; grille figée conservée avec cadence de rafraîchissement écrite dans le code (pas seulement une intention) et dérive courante affichée. Détail et arbitrage : dette technique `IGRF-GRID-DERIVE-RENOUVELEE-001` (registre interne).
